...
export function list_iterator_new(list: list_t | null, direction: list_direction_t): list_iterator_t | null {
  let node = (direction == LIST_HEAD ? (__struct_ptr_at(list, 0)).head : (__struct_ptr_at(list, 0)).tail); /* &ref */
  return list_iterator_new_from_node(node, direction);
}

export function list_iterator_new_from_node(node: list_node_t | null, direction: list_direction_t): list_iterator_t | null {
  let self = null;
  if (!(self = (new list_iterator_t()))) {
    return null;
  }
  (__struct_ptr_at(self, 0)).next = node;
  (__struct_ptr_at(self, 0)).direction = direction;
  return self;
}

export function list_iterator_next(self: list_iterator_t | null): list_node_t | null {
  let curr = (__struct_ptr_at(self, 0)).next; /* &ref */
  if (curr) {
    (__struct_ptr_at(self, 0)).next = ((__struct_ptr_at(self, 0)).direction == LIST_HEAD ? (__struct_ptr_at(curr, 0)).next : (__struct_ptr_at(curr, 0)).prev);
  }
  return curr;
}

export function list_iterator_destroy(self: list_iterator_t | null): void {
  free(self);
  self = null;
}
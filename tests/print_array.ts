...
function assert_print_array(expected: string, input: string): void {
  ...
  memset(item, 0, 64);
  if ((parse_array(item, parsebuffer))) {
  } else {
    UnityFail(((("Failed to parse array."))), __u64(__as_bigint((__u64(__as_bigint((57)))))));
  }
  unformatted_buffer.format = (Math.trunc(+(0)));
  if ((print_array(item, unformatted_buffer))) {
  } else {
    UnityFail(((("Failed to print unformatted string."))), __u64(__as_bigint((__u64(__as_bigint((60)))))));
  }
  UnityAssertEqualString(cptr_clone((((input)))), cptr_clone((((printed_unformatted)))), (("Unformatted array is not correct.")), __u64(__as_bigint((61))));
  formatted_buffer.format = (Math.trunc(+(1)));
  if ((print_array(item, formatted_buffer))) {
  } else {
    UnityFail(((("Failed to print formatted string."))), __u64(__as_bigint((__u64(__as_bigint((64)))))));
  }
  UnityAssertEqualString(cptr_clone((((expected)))), cptr_clone((((printed_formatted)))), (("Formatted array is not correct.")), __u64(__as_bigint((65))));
  reset(item);
}

function print_array_should_print_empty_arrays(): void {
  assert_print_array("[]", "[]");
}

function print_array_should_print_arrays_with_one_element(): void {
...
function print_array_should_print_arrays_with_multiple_elements(): void {
  assert_print_array("[1, 2, 3]", "[1,2,3]");
  assert_print_array("[1, null, true, false, [], \"hello\", {\n\t}]", "[1,null,true,false,[],\"hello\",{}]");
}

export function main(): number {
  UnityBegin("tests/real-world/cjson/tests/print_array.c");
  UnityDefaultTestRun((print_array_should_print_empty_arrays), "print_array_should_print_empty_arrays", (95));
  UnityDefaultTestRun((print_array_should_print_arrays_with_one_element), "print_array_should_print_arrays_with_one_element", (96));
  UnityDefaultTestRun((print_array_should_print_arrays_with_multiple_elements), "print_array_should_print_arrays_with_multiple_elements", (97));
  return UnityEnd();
}

...
import { UnityAssertEqualString, UnityBegin, UnityDefaultTestRun, UnityEnd, UnityFail, UnityIgnore } from './unity.js';
...
function assert_print_number(expected: string, input: number): void {
  let printed = cptr_create(1024);
  let new_buffer = cptr_create(26);
  let i = ((0) >>> 0);
  let item = new cJSON();
  let buffer = Object.assign(new printbuffer(), { buffer: null, length: ((0) >>> 0), offset: ((0) >>> 0), depth: ((0) >>> 0), noalloc: 0, format: 0, hooks: Object.assign(new internal_hooks(), { allocate: null, deallocate: null, reallocate: null }) });
  buffer.buffer = printed;
  buffer.length = 1024;
  buffer.offset = ((0) >>> 0);
  buffer.noalloc = (Math.trunc(+(1)));
  Object.assign(buffer.hooks, global_hooks);
  buffer.buffer = new_buffer;
  memset(item, 0, 64);
  memset(new_buffer, 0, 26);
  (((item != (null)) ? cJSON_SetNumberHelper(item, (input)) : (input)));
  if ((print_number(item, buffer))) {
  } else {
    UnityFail(((("Failed to print number."))), __u64(__as_bigint((__u64(__as_bigint((44)))))));
  }
  for (i = ((0) >>> 0); ((i) >>> 0) < 26; (() => { const _t = i; i = u32(i + 1); return _t; })()) {
    if (((((i) >>> 0) > ((3) >>> 0) && ((new_buffer.buf[(new_buffer.off ?? 0) + ((i) >>> 0)]) & 0xFF) == 48) ? 1 : 0)) {
      if ((((((((((new_buffer.buf[(new_buffer.off ?? 0) + u32(((i) >>> 0) - ((3) >>> 0))]) & 0xFF) == 101 && ((new_buffer.buf[(new_buffer.off ?? 0) + u32(((i) >>> 0) - ((2) >>> 0))]) & 0xFF) == 45) ? 1 : 0) && ((new_buffer.buf[(new_buffer.off ?? 0) + ((i) >>> 0)]) & 0xFF) == 48) ? 1 : 0)) || (((((new_buffer.buf[(new_buffer.off ?? 0) + u32(((i) >>> 0) - ((2) >>> 0))]) & 0xFF) == 101 && ((new_buffer.buf[(new_buffer.off ?? 0) + u32(((i) >>> 0) - ((1) >>> 0))]) & 0xFF) == 43) ? 1 : 0))) ? 1 : 0)) {
        while (((new_buffer.buf[(new_buffer.off ?? 0) + ((i) >>> 0)]) & 0xFF) != 0) {
          new_buffer.buf[(new_buffer.off ?? 0) + ((i) >>> 0)] = (((new_buffer.buf[(new_buffer.off ?? 0) + u32(((i) >>> 0) + ((1) >>> 0))]) & 0xFF)) & 0xFF;
          (() => { const _t = i; i = u32(i + 1); return _t; })();
        }
      }
    }
  }
  UnityAssertEqualString(cptr_clone((((expected)))), cptr_clone((((buffer.buffer)))), (("Printed number is not as expected.")), __u64(__as_bigint((62))));
}

function print_number_should_print_zero(): void {
  assert_print_number("0", 0);
}

function print_number_should_print_negative_integers(): void {
  assert_print_number("-1", -1);
  assert_print_number("-32768", -32768);
  assert_print_number("-2147483648", -2147483648);
}

function print_number_should_print_positive_integers(): void {
  assert_print_number("1", 1);
  assert_print_number("32767", 32767);
  assert_print_number("2147483647", 2147483647);
}

function print_number_should_print_positive_reals(): void {
  assert_print_number("0.123", 0.123);
  assert_print_number("1e-09", 1.0000000000000001E-9);
  assert_print_number("1000000000000", 1.0E+12);
  assert_print_number("1.23e+129", 1.23E+129);
  assert_print_number("1.23e-126", 1.2299999999999999E-126);
  assert_print_number("3.1415926535897931", 3.1415926535897931);
}

function print_number_should_print_negative_reals(): void {
  assert_print_number("-0.0123", -0.0123);
  assert_print_number("-1e-09", -1.0000000000000001E-9);
  assert_print_number("-1e+21", -1.0E+21);
  assert_print_number("-1.23e+129", -1.23E+129);
  assert_print_number("-1.23e-126", -1.2299999999999999E-126);
}

function print_number_should_print_non_number(): void {
  UnityIgnore(null, __u64(__as_bigint((105))));
}

export function main(): number {
  UnityBegin("tests/real-world/cjson/tests/print_number.c");
  UnityDefaultTestRun((print_number_should_print_zero), "print_number_should_print_zero", (117));
  UnityDefaultTestRun((print_number_should_print_negative_integers), "print_number_should_print_negative_integers", (118));
  UnityDefaultTestRun((print_number_should_print_positive_integers), "print_number_should_print_positive_integers", (119));
  UnityDefaultTestRun((print_number_should_print_positive_reals), "print_number_should_print_positive_reals", (120));
  UnityDefaultTestRun((print_number_should_print_negative_reals), "print_number_should_print_negative_reals", (121));
  UnityDefaultTestRun((print_number_should_print_non_number), "print_number_should_print_non_number", (122));
  return UnityEnd();
}

...
export function crc32_for_byte(_byte: number): number {
  let polynomial = ((3988292384) >>> 0);
  let result = ((_byte) >>> 0);
  let i = ((0) >>> 0);
  for (; (((i) >>> 0) < ((8) >>> 0) ? 1 : 0); (() => { const _t = i; i = u32(i + 1); return _t; })()) {
    result = (((((result) >>> 0) >>> 1) >>> 0) ^ (Math.imul(((((result) >>> 0) & ((1) >>> 0)) >>> 0), ((polynomial) >>> 0)) >>> 0)) >>> 0;
  }
  return ((result) >>> 0);
}

export function crc32(input: any | null, size: number): number {
  let current = cptr_clone(cptr_clone(input)); /* &ref */
  let result = 4294967295;
  let i = ((0) >>> 0);
  for (; (((i) >>> 0) < ((size) >>> 0) ? 1 : 0); (() => { const _t = i; i = u32(i + 1); return _t; })()) {
    result = (result ^ ((((current.buf[(current.off ?? 0) + ((i) >>> 0)]) & 0xFF)) >>> 0)) >>> 0;
    result = crc32_for_byte(((result) >>> 0));
  }
  return (~((result) >>> 0)) >>> 0;
}

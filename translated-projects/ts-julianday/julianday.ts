...
export function date_to_jd(year: any, month: any, day: any): number {
  let a = (__safe_div((i32(14 - month)), 12));
  let y = (i32(i32(year + 4800) - a));
  let m = (i32(i32(month + (Math.imul(12, a))) - 3));
  return (i32(i32(i32(i32(i32(i32(day + (__safe_div((i32((Math.imul(153, m)) + 2)), 5))) + (Math.imul(y, 365))) + (__safe_div(y, 4))) - (__safe_div(y, 100))) + (__safe_div(y, 400))) - 32045));
}

export function jd_to_date(jd: number, year: any | null, month: any | null, day: any | null): void {
  let a = i32(jd + 32044);
  let b = (__safe_div((i32((Math.imul(4, a)) + 3)), 146097));
  let c = i32(a - (__safe_div((Math.imul(b, 146097)), 4)));
  let d = (__safe_div((i32((Math.imul(4, c)) + 3)), 1461));
  let e = i32(c - (__safe_div((Math.imul(d, 1461)), 4)));
  let m = (__safe_div((i32((Math.imul(5, e)) + 2)), 153));
  let m2 = (__safe_div(m, 10));
  cptr_write_int32(day, 0, (i32(i32(e + 1) - (__safe_div((i32((Math.imul(153, m)) + 2)), 5)))));
  cptr_write_int32(month, 0, (i32(i32(m + 3) - (Math.imul(12, m2)))));
  cptr_write_int32(year, 0, (i32(i32(i32((Math.imul(b, 100)) + d) - 4800) + m2)));
}

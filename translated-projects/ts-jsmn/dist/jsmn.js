...
function strncmp(a, b, n) { const sa = ((typeof a === 'string') ? a : (a?.buf ? cptr_to_string(a) : a?.toString?.() ?? '')).substring(0, n); const sb = ((typeof b === 'string') ? b : (b?.buf ? cptr_to_string(b) : b?.toString?.() ?? '')).substring(0, n); return sa < sb ? -1 : sa > sb ? 1 : 0; }
function write(fd, buf, count) { if (fd === 1) {
    const s = (buf?.buf) ? cptr_to_string(buf) : (typeof buf === 'string' ? buf : String.fromCharCode(...new Uint8Array(buf, 0, count)));
    process.stdout.write(s.substring(0, count));
    return count;
} if (fd === 2) {
    const s = (buf?.buf) ? cptr_to_string(buf) : (typeof buf === 'string' ? buf : String(buf));
    process.stderr.write(s.substring(0, count));
    return count;
} try {
    const data = (buf?.buf) ? Buffer.from(buf.buf.buffer, buf.buf.byteOffset + buf.off, count) : (typeof buf === 'string' ? Buffer.from(buf) : Buffer.from(buf));
    require('fs').writeSync(fd, data, 0, count);
    return count;
}
catch {
...
function includes(first1, last1, first2, last2, comp) { const A = __cpp_arr(first1, last1); const B = __cpp_arr(first2, last2); const lt = comp ?? ((a, b) => a < b); let i = A.start, j = B.start; while (i < A.end && j < B.end) {
    if (lt(B.arr[j], A.arr[i]))
        return false;
    if (lt(A.arr[i], B.arr[j]))
        i++;
    else {
        i++;
        j++;
...
function __struct_ptr_advance(p, n) { if (p == null)
    return p; const delta = Number(n) | 0; if (Array.isArray(p))
    return { __arr: p, __idx: delta }; if (p && p.__arr !== undefined)
    return { __arr: p.__arr, __idx: (p.__idx ?? 0) + delta }; if (p && p.__cptr_overlay === true) {
    return cptr_struct_overlay(p.__structT, p.__cptr, (p.__byteOff ?? 0) + delta * (p.__layout?.totalSize ?? 0));
} if (p && p.buf)
    return { buf: p.buf, off: (p.off ?? 0) + delta }; return p; }
function printf(fmt, ...args) {
    ...
    while (i < fmt.length) {
        if (fmt[i] === "%" && i + 1 < fmt.length) {
            ...
            const toUnsignedStr = (v, radix, upper) => {
                if (typeof v === "bigint") {
            ...
            switch (spec) {
                ...
                case "i": {
                    if (typeof av === "bigint") {
                        const n = is64 ? BigInt.asIntN(64, av) : av;
                        let mag = (n < 0n ? -n : n).toString();
                        if (prec) {
                            const p = parseInt(prec);
                            if (mag.length < p)
                                mag = mag.padStart(p, "0");
                        }
                        s = (n < 0n ? "-" : "") + mag;
                        if (n >= 0n && flags.includes("+"))
                            s = "+" + s;
                        else if (n >= 0n && flags.includes(" "))
                            s = " " + s;
                        break;
                    }
                    const n = Math.trunc(Number(av));
                    let mag = Math.abs(n).toString();
                    if (prec) {
                        const p = parseInt(prec);
                        if (mag.length < p)
                            mag = mag.padStart(p, "0");
                ...
                case "u":
                    ...
                    break;
                case "x":
                    s = toUnsignedStr(av, 16, false);
                    if (prec) {
                        const p = parseInt(prec);
                        if (s.length < p)
                            s = s.padStart(p, "0");
                    }
                    if (flags.includes("#") && s !== "0")
                        s = "0x" + s;
                    break;
                case "X":
                    s = toUnsignedStr(av, 16, true);
                    if (prec) {
                        const p = parseInt(prec);
                        if (s.length < p)
                            s = s.padStart(p, "0");
                    ...
                    break;
                case "o":
                    s = toUnsignedStr(av, 8, false);
                    if (prec) {
                        const p = parseInt(prec);
                        if (s.length < p)
                            s = s.padStart(p, "0");
                    }
                    if (flags.includes("#") && !s.startsWith("0"))
                        s = "0" + s;
                    break;
                case "f":
                case "F": {
                    const aObj = av;
                    const nn = Number(aObj);
                    if (Number.isNaN(nn)) {
                        s = spec === "F" ? "NAN" : "nan";
                        break;
                    }
                    if (!Number.isFinite(nn)) {
                        s = (nn < 0 ? "-" : (flags.includes("+") ? "+" : (flags.includes(" ") ? " " : ""))) + (spec === "F" ? "INF" : "inf");
                        break;
                    }
                    const isDec = !!(aObj && typeof aObj.toFixed === "function" && typeof aObj.toNumber === "function");
                    ...
                    const isNegZero = !isDec && Object.is(nn, -0);
                    const sign = isDec ? (aObj.isNegative && aObj.isNegative() ? -1 : 1) : (nn < 0 || isNegZero ? -1 : 1);
                    if (isNegZero && !s.startsWith("-"))
                        s = "-" + s;
                    if (sign >= 0 && flags.includes("+"))
                        s = "+" + s;
                    else if (sign >= 0 && flags.includes(" "))
                        s = " " + s;
                    break;
                }
                case "e": {
                    ...
                    if (!Number.isFinite(nn)) {
                        s = (nn < 0 ? "-" : (flags.includes("+") ? "+" : (flags.includes(" ") ? " " : ""))) + "inf";
                        break;
                    }
                    const isDec = !!(aObj && typeof aObj.toExponential === "function" && typeof aObj.toNumber === "function");
                    const p = prec ? parseInt(prec) : 6;
                    if (isDec) {
                        s = aObj.toExponential(p);
                    }
                    else {
                        s = nn.toExponential(p);
                    }
                    s = s.replace(/e([+-])(\d)$/, "e$1" + "0$2");
                    const sign = isDec ? (aObj.isNegative && aObj.isNegative() ? -1 : 1) : (nn < 0 ? -1 : 1);
                    if (sign >= 0 && flags.includes("+"))
                        s = "+" + s;
                    else if (sign >= 0 && flags.includes(" "))
                        s = " " + s;
                ...
                case "E": {
                    ...
                    s = s.replace(/e([+-])(\d)$/, "e$1" + "0$2").toUpperCase();
                    const sign = isDec ? (aObj.isNegative && aObj.isNegative() ? -1 : 1) : (nn < 0 ? -1 : 1);
                    if (sign >= 0 && flags.includes("+"))
                        s = "+" + s;
                    else if (sign >= 0 && flags.includes(" "))
                ...
                case "G": {
                    ...
                    if (!Number.isFinite(nn)) {
                        s = (nn < 0 ? "-" : (flags.includes("+") ? "+" : (flags.includes(" ") ? " " : ""))) + (spec === "G" ? "INF" : "inf");
                        break;
                    }
                    const isDec = !!(aObj && typeof aObj.toPrecision === "function" && typeof aObj.toNumber === "function");
                    const gPrec = prec ? parseInt(prec) : 6;
                    if (isDec) {
                        s = aObj.toPrecision(gPrec);
                    }
                    else {
                        s = nn.toPrecision(gPrec);
                    }
                    s = s.replace(/(\.\d*?)0+(?=e|$)/, "$1").replace(/\.(?=e|$)/, "").replace(/e([+-])(\d)$/, "e$1" + "0$2");
                    if (spec === "G")
                        s = s.toUpperCase();
                    const sign = isDec ? (aObj.isNegative && aObj.isNegative() ? -1 : 1) : (nn < 0 ? -1 : 1);
                    if (sign >= 0 && flags.includes("+"))
                ...
                case "A": {
                    const v = Number(av);
                    if (v === 0) {
                        s = (spec === "A" ? "0X0P+0" : "0x0p+0");
                        break;
                    }
                    const __bb = new ArrayBuffer(8);
                    new DataView(__bb).setFloat64(0, v, true);
                    const __dv = new DataView(__bb);
                    const __lo = __dv.getUint32(0, true), __hi = __dv.getUint32(4, true);
                    const __sg = (__hi >>> 31) & 1;
                    const __ex = ((__hi >>> 20) & 0x7FF) - 1023;
                    let __m = (__hi & 0xFFFFF).toString(16).padStart(5, "0") + __lo.toString(16).padStart(8, "0");
                    __m = __m.replace(/0+$/, "");
                    const __exStr = __ex >= 0 ? ("+" + __ex) : String(__ex);
                    const __head = spec === "A" ? "0X1" : "0x1";
                    const __mid = __m ? ("." + (spec === "A" ? __m.toUpperCase() : __m)) : "";
                    const __p = spec === "A" ? "P" : "p";
                    s = (__sg ? "-" : "") + __head + __mid + __p + __exStr;
                    break;
                }
                case "c":
                    s = typeof av === "string" ? av.charAt(0) : String.fromCharCode(Number(av));
                    break;
                case "p":
                    s = "0x" + (Number(av) >>> 0).toString(16);
                    break;
                case "n": {
                    if (a?.buf)
                        new DataView(a.buf.buffer, a.buf.byteOffset).setInt32(a.off ?? 0, result.length, true);
                    else if (a && typeof a === "object" && "value" in a)
                        a.value = result.length;
                    s = "";
                    break;
                }
                case "%":
            ...
            }
            {
                ...
                if (ww > s.length) {
                    const zero = flags.includes("0") && !leftAlign && "diouxXeEfFgG".includes(spec);
                    const padChar = zero ? "0" : " ";
                    const padLen = ww - s.length;
                    const pad = padChar.repeat(padLen); /* C17 §7.21.6.1: zero-pad goes BETWEEN the sign and the magnitude, not before the sign. Same for 0x/0X prefixes added by # flag. */
                    if (zero && !leftAlign && (s.startsWith("-") || s.startsWith("+") || s.startsWith(" "))) {
                        s = s[0] + pad + s.slice(1);
                    }
                    else if (zero && !leftAlign && (s.startsWith("0x") || s.startsWith("0X"))) {
                        s = s.slice(0, 2) + pad + s.slice(2);
                    }
                    else {
    ...
    const __fh = globalThis._fileHandles;
    if (typeof stdout === "object" && stdout && typeof stdout.__fd === "number" && __fh) {
        const h = __fh.get(stdout.__fd);
        if (h) {
...
function strlen(s) { if (s == null)
    return 0; if (typeof s === 'string')
    return s.length; if (s.buf)
    return cptr_strlen(s); if (Array.isArray(s)) {
    let i = 0;
    while (s[i] !== 0 && s[i] !== undefined && i < s.length)
        i++;
    return i;
} return s?.length ?? 0; }
export const JSMN_UNDEFINED = 0;
export const JSMN_OBJECT = ((1 << 0) | 0);
export const JSMN_ARRAY = ((1 << 1) | 0);
export const JSMN_STRING = ((1 << 2) | 0);
export const JSMN_PRIMITIVE = ((1 << 3) | 0);
export const JSMN_ERROR_NOMEM = -1;
export const JSMN_ERROR_INVAL = -2;
export const JSMN_ERROR_PART = -3;
...
jsmntok.__fieldTypes = ["int32", "int32", "int32", "int32"];
jsmntok.__fieldNames = ["type", "start", "end", "size"];
jsmntok.__fieldOffsets = [0, 8, 12, 16];
// BRIDGE: struct-as-class — C17 §6.7.2.1 / C++20 [class]
...
jsmn_parser.__fieldTypes = ["int32", "int32", "int32"];
jsmn_parser.__fieldNames = ["pos", "toknext", "toksuper"];
jsmn_parser.__fieldOffsets = [0, 4, 8];
function jsmn_alloc_token(parser, tokens, num_tokens) {
    let tok = null;
...
function jsmn_fill_token(token, type, start, end) {
...
function jsmn_parse_primitive(parser, js, len, tokens, num_tokens) {
    if (typeof js === 'string')
        js = cptr_from_string(js);
    let token = null;
    let start = 0;
    let _state = 0;
    _sm: while (true) {
...
function jsmn_parse_string(parser, js, len, tokens, num_tokens) {
    if (typeof js === 'string')
        js = cptr_from_string(js);
    let token = null;
...
export function jsmn_parse(parser, js, len, tokens, num_tokens) {
    if (typeof js === 'string')
        js = cptr_from_string(js);
    let r = 0;
    let i = 0;
...
export function jsmn_init(parser) {

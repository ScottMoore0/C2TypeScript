...
let crc_tab16_init = (0 ? 1 : 0);
let crc_tab16 = new Array(256).fill(0);
export function crc_16(input_str: any | null, num_bytes: number): number {
  let crc = 0;
  let ptr = null;
  let a = 0;
  if ((!crc_tab16_init ? 1 : 0)) {
    init_crc16_tab();
  }
  crc = (((0) & 0xFFFF)) & 0xFFFF;
  ptr = cptr_clone(input_str);
  if ((!cptr_eq(ptr, (null)) ? 1 : 0)) {
    for (a = ((0) >>> 0); (((a) >>> 0) < ((num_bytes) >>> 0) ? 1 : 0); (() => { const _t = a; a = u32(a + 1); return _t; })()) {
      crc = ((((((((crc) & 0xFFFF) >> 8) | 0)) ^ ((crc_tab16[(((crc) & 0xFFFF) ^ ((Math.trunc(+((((ptr.buf[ptr.off++])) & 0xFF)))) & 0xFFFF)) & 255]) & 0xFFFF)) & 0xFFFF)) & 0xFFFF;
    }
  }
  return ((crc) & 0xFFFF);
}

export function crc_modbus(input_str: any | null, num_bytes: number): number {
  let crc = 0;
  let ptr = null;
  let a = 0;
  if ((!crc_tab16_init ? 1 : 0)) {
...
export function update_crc_16(crc: number, c: number): number {
  if ((!crc_tab16_init ? 1 : 0)) {
    init_crc16_tab();
  }
  return (((((((crc) & 0xFFFF) >> 8) | 0)) ^ ((crc_tab16[(((crc) & 0xFFFF) ^ ((Math.trunc(+(((c) & 0xFF)))) & 0xFFFF)) & 255]) & 0xFFFF)) & 0xFFFF);
}

function init_crc16_tab(): void {
  let i = 0;
  let j = 0;
  let crc = 0;
  let c = 0;
  for (i = (((0) & 0xFFFF)) & 0xFFFF; (((i) & 0xFFFF) < 256 ? 1 : 0); (() => { const _t = i; i = u32(i + 1); return _t; })()) {
    crc = (((0) & 0xFFFF)) & 0xFFFF;
    c = (((i) & 0xFFFF)) & 0xFFFF;
    for (j = (((0) & 0xFFFF)) & 0xFFFF; (((j) & 0xFFFF) < 8 ? 1 : 0); (() => { const _t = j; j = u32(j + 1); return _t; })()) {
      if ((((crc) & 0xFFFF) ^ ((c) & 0xFFFF)) & 1) {
        crc = ((((((((crc) & 0xFFFF) >> 1) | 0)) ^ 40961) & 0xFFFF)) & 0xFFFF;
      } else {
        crc = (((((((crc) & 0xFFFF) >> 1) | 0)) & 0xFFFF)) & 0xFFFF;
      }
      c = (((((((c) & 0xFFFF) >> 1) | 0)) & 0xFFFF)) & 0xFFFF;
    }
    crc_tab16[((i) & 0xFFFF)] = (((crc) & 0xFFFF)) & 0xFFFF;
  }
  crc_tab16_init = (1 ? 1 : 0);
}

let crc_tabccitt_init = (0 ? 1 : 0);
let crc_tabccitt = new Array(256).fill(0);
export function crc_xmodem(input_str: any | null, num_bytes: number): number {
  return crc_ccitt_generic(cptr_clone(input_str), ((num_bytes) >>> 0), ((0) & 0xFFFF));
}

export function crc_ccitt_1d0f(input_str: any | null, num_bytes: number): number {
  return crc_ccitt_generic(cptr_clone(input_str), ((num_bytes) >>> 0), ((7439) & 0xFFFF));
}

export function crc_ccitt_ffff(input_str: any | null, num_bytes: number): number {
  return crc_ccitt_generic(cptr_clone(input_str), ((num_bytes) >>> 0), ((65535) & 0xFFFF));
}

function crc_ccitt_generic(input_str: any | null, num_bytes: number, start_value: number): number {
  let crc = 0;
  let ptr = null;
  let a = 0;
  if ((!crc_tabccitt_init ? 1 : 0)) {
    init_crcccitt_tab();
  }
  crc = (((start_value) & 0xFFFF)) & 0xFFFF;
  ptr = cptr_clone(input_str);
  if ((!cptr_eq(ptr, (null)) ? 1 : 0)) {
    for (a = ((0) >>> 0); (((a) >>> 0) < ((num_bytes) >>> 0) ? 1 : 0); (() => { const _t = a; a = u32(a + 1); return _t; })()) {
      crc = ((((((((crc) & 0xFFFF) << 8) | 0)) ^ ((crc_tabccitt[((((((crc) & 0xFFFF) >> 8) | 0)) ^ ((Math.trunc(+((((ptr.buf[ptr.off++])) & 0xFF)))) & 0xFFFF)) & 255]) & 0xFFFF)) & 0xFFFF)) & 0xFFFF;
    }
  }
  return ((crc) & 0xFFFF);
}

export function update_crc_ccitt(crc: number, c: number): number {
  if ((!crc_tabccitt_init ? 1 : 0)) {
    init_crcccitt_tab();
  }
  return (((((((crc) & 0xFFFF) << 8) | 0)) ^ ((crc_tabccitt[((((((crc) & 0xFFFF) >> 8) | 0)) ^ ((Math.trunc(+(((c) & 0xFF)))) & 0xFFFF)) & 255]) & 0xFFFF)) & 0xFFFF);
}

function init_crcccitt_tab(): void {
  let i = 0;
  let j = 0;
  let crc = 0;
  let c = 0;
  for (i = (((0) & 0xFFFF)) & 0xFFFF; (((i) & 0xFFFF) < 256 ? 1 : 0); (() => { const _t = i; i = u32(i + 1); return _t; })()) {
    crc = (((0) & 0xFFFF)) & 0xFFFF;
    c = (((((((i) & 0xFFFF) << 8) | 0)) & 0xFFFF)) & 0xFFFF;
    for (j = (((0) & 0xFFFF)) & 0xFFFF; (((j) & 0xFFFF) < 8 ? 1 : 0); (() => { const _t = j; j = u32(j + 1); return _t; })()) {
      if ((((crc) & 0xFFFF) ^ ((c) & 0xFFFF)) & 32768) {
        crc = ((((((((crc) & 0xFFFF) << 1) | 0)) ^ 4129) & 0xFFFF)) & 0xFFFF;
      } else {
        crc = (((((((crc) & 0xFFFF) << 1) | 0)) & 0xFFFF)) & 0xFFFF;
      }
      c = (((((((c) & 0xFFFF) << 1) | 0)) & 0xFFFF)) & 0xFFFF;
    }
    crc_tabccitt[((i) & 0xFFFF)] = (((crc) & 0xFFFF)) & 0xFFFF;
  }
  crc_tabccitt_init = (1 ? 1 : 0);
}

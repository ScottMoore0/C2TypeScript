...
function assert_print_object(expected: string, input: string): void {
  let printed_unformatted = cptr_create(1024);
  let printed_formatted = cptr_create(1024);
  let item = new cJSON();
  let formatted_buffer = Object.assign(new printbuffer(), { buffer: null, length: ((0) >>> 0), offset: ((0) >>> 0), depth: ((0) >>> 0), noalloc: 0, format: 0, hooks: Object.assign(new internal_hooks(), { allocate: null, deallocate: null, reallocate: null }) });
  let unformatted_buffer = Object.assign(new printbuffer(), { buffer: null, length: ((0) >>> 0), offset: ((0) >>> 0), depth: ((0) >>> 0), noalloc: 0, format: 0, hooks: Object.assign(new internal_hooks(), { allocate: null, deallocate: null, reallocate: null }) });
  ...
  parsebuffer.content = (input);
  parsebuffer.length = strlen(cptr_clone(input)) + 1;
  Object.assign(parsebuffer.hooks, global_hooks);
  formatted_buffer.buffer = printed_formatted;
  formatted_buffer.length = 1024;
  formatted_buffer.offset = ((0) >>> 0);
  formatted_buffer.noalloc = (Math.trunc(+(1)));
  Object.assign(formatted_buffer.hooks, global_hooks);
  unformatted_buffer.buffer = printed_unformatted;
  unformatted_buffer.length = 1024;
  unformatted_buffer.offset = ((0) >>> 0);
  unformatted_buffer.noalloc = (Math.trunc(+(1)));
  Object.assign(unformatted_buffer.hooks, global_hooks);
  memset(item, 0, 64);
  if ((parse_object(item, parsebuffer))) {
  } else {
    UnityFail(((("Failed to parse object."))), __u64(__as_bigint((__u64(__as_bigint((58)))))));
  }
  unformatted_buffer.format = (Math.trunc(+(0)));
  if ((print_object(item, unformatted_buffer))) {
  } else {
    UnityFail(((("Failed to print unformatted string."))), __u64(__as_bigint((__u64(__as_bigint((61)))))));
  }
  UnityAssertEqualString(cptr_clone((((input)))), cptr_clone((((printed_unformatted)))), (("Unformatted object is not correct.")), __u64(__as_bigint((62))));
  formatted_buffer.format = (Math.trunc(+(1)));
  if ((print_object(item, formatted_buffer))) {
  } else {
    UnityFail(((("Failed to print formatted string."))), __u64(__as_bigint((__u64(__as_bigint((65)))))));
  }
  UnityAssertEqualString(cptr_clone((((expected)))), cptr_clone((((printed_formatted)))), (("Formatted object is not correct.")), __u64(__as_bigint((66))));
  reset(item);
}

function print_object_should_print_empty_objects(): void {
  assert_print_object("{\n}", "{}");
}

function print_object_should_print_objects_with_one_element(): void {
  assert_print_object("{\n\t\"one\":\t1\n}", "{\"one\":1}");
  assert_print_object("{\n\t\"hello\":\t\"world!\"\n}", "{\"hello\":\"world!\"}");
  assert_print_object("{\n\t\"array\":\t[]\n}", "{\"array\":[]}");
  assert_print_object("{\n\t\"null\":\tnull\n}", "{\"null\":null}");
...
function print_object_should_print_objects_with_multiple_elements(): void {
  assert_print_object("{\n\t\"one\":\t1,\n\t\"two\":\t2,\n\t\"three\":\t3\n}", "{\"one\":1,\"two\":2,\"three\":3}");
  assert_print_object("{\n\t\"one\":\t1,\n\t\"NULL\":\tnull,\n\t\"TRUE\":\ttrue,\n\t\"FALSE\":\tfalse,\n\t\"array\":\t[],\n\t\"world\":\t\"hello\",\n\t\"object\":\t{\n\t}\n}", "{\"one\":1,\"NULL\":null,\"TRUE\":true,\"FALSE\":false,\"array\":[],\"world\":\"hello\",\"object\":{}}");
}

export function main(): number {
  UnityBegin("tests/real-world/cjson/tests/print_object.c");
  UnityDefaultTestRun((print_object_should_print_empty_objects), "print_object_should_print_empty_objects", (96));
  UnityDefaultTestRun((print_object_should_print_objects_with_one_element), "print_object_should_print_objects_with_one_element", (97));
  UnityDefaultTestRun((print_object_should_print_objects_with_multiple_elements), "print_object_should_print_objects_with_multiple_elements", (98));
  return UnityEnd();
}

import { Unity, UnityBegin, UnityEnd, UnityPrint, UnityPrintNumberUnsigned } from './unity.js';
...
function strstr(h: any, n: any): any { if (h == null || n == null) return null; const hs = typeof h === 'string' ? h : h?.buf ? cptr_to_string(h) : h?.toString?.() ?? ''; const ns = typeof n === 'string' ? n : n?.buf ? cptr_to_string(n) : n?.toString?.() ?? ''; const idx = hs.indexOf(ns); if (idx < 0) return null; if (h?.buf) return { buf: h.buf, off: h.off + idx }; const p = cptr_from_string(hs); p.off = idx; return p; }
...
(UNITY_FIXTURE_T as any).__fieldTypes = ["int32","int32","int64","int64"];
(UNITY_FIXTURE_T as any).__fieldNames = ["Verbose","RepeatCount","NameFilter","GroupFilter"];
(UNITY_FIXTURE_T as any).__fieldOffsets = [0,4,8,16];

type unityfunction = (...args: any[]) => any;
export let UnityFixture = new UNITY_FIXTURE_T();
...
function announceTestRun(runNumber: number): void {
  UnityPrint("Unity test run ");
  UnityPrintNumberUnsigned(u32(((runNumber) >>> 0) + ((1) >>> 0)));
  UnityPrint(" of ");
  UnityPrintNumberUnsigned(((UnityFixture.RepeatCount) >>> 0));
  UNITY_PRINT_EOL();
}

export function UnityMain(argc: number, argv: { value: string }, runAllTests: (arg0: void) => void): number {
  let result = UnityGetCommandLineOptions(argc, cptr_clone(argv));
  let r = 0;
  if (result != 0) {
    return result;
  }
  for (r = ((0) >>> 0); ((r) >>> 0) < ((UnityFixture.RepeatCount) >>> 0); (() => { const _t = r; r = u32(r + 1); return _t; })()) {
    UnityBegin(cptr_clone(cptr_read_ptr(argv, 0)));
    announceTestRun(((r) >>> 0));
    runAllTests();
...
function selected(filter: string, name: string): number {
  if (filter == null) {
    return 1;
  }
  return (strstr(cptr_clone(name), cptr_clone(filter)) ? 1 : 0);
}

function testSelected(test: string): number {
  return selected(cptr_clone(UnityFixture.NameFilter), cptr_clone(test));
}

function groupSelected(group: string): number {
  return selected(cptr_clone(UnityFixture.GroupFilter), cptr_clone(group));
}

export function UnityTestRunner(setup: unityfunction | null, testBody: unityfunction | null, teardown: unityfunction | null, printableName: string, group: string, name: string, file: string, line: number): void {
  if (((testSelected(cptr_clone(name)) && groupSelected(cptr_clone(group))) ? 1 : 0)) {
    if (!UnityFixture.Verbose) {
      UNITY_OUTPUT_CHAR(46);
    } else {
      UnityPrint(cptr_clone(printableName));
    }
...
export function UnityIgnoreTest(printableName: string, group: string, name: string): void {
  if (((testSelected(cptr_clone(name)) && groupSelected(cptr_clone(group))) ? 1 : 0)) {
    if (!UnityFixture.Verbose) {
      UNITY_OUTPUT_CHAR(33);
    } else {
      UnityPrint(cptr_clone(printableName));
      UNITY_PRINT_EOL();
    }
...
let malloc_count = 0;
let malloc_fail_countdown = -1;
export function UnityMalloc_StartTest(): void {
  malloc_count = 0;
  malloc_fail_countdown = -1;
}

export function UnityMalloc_EndTest(): void {
  malloc_fail_countdown = -1;
  if (malloc_count != 0) {
    UNITY_TEST_FAIL("This test leaks!");
  }
}

export function UnityMalloc_MakeMallocFailAfterCount(countdown: number): void {
  malloc_fail_countdown = countdown;
}

// BRIDGE: struct-as-class — C17 §6.7.2.1 / C++20 [class]
export class GuardBytes {
  size: number;
  guard_space: number;
  constructor() {
    this.size = 0;
...
const Guard = GuardBytes;
type Guard = GuardBytes;
(GuardBytes as any).__fieldTypes = ["int64","int64"];
(GuardBytes as any).__fieldNames = ["size","guard_space"];
(GuardBytes as any).__fieldOffsets = [0,8];

const end = (() => { const __b = cptr_create(4); const __s = "END"; for (let __i = 0; __i < __s.length; __i++) __b.buf[__i] = __s.charCodeAt(__i); return __b; })();
export function unity_malloc(size: number): any | null {
  let mem = null;
  let guard = null;
  let total_size = __u64(__as_bigint(((size) >>> 0) + 16) + __as_bigint(4));
  if (malloc_fail_countdown != -1) {
  ...
  malloc_count++;
  (__struct_ptr_at(guard, 0)).size = ((size) >>> 0);
  (__struct_ptr_at(guard, 0)).guard_space = ((0) >>> 0);
  mem = (__struct_ptr_advance(guard, 1));
  memcpy(cptr_offset(mem, (((size) >>> 0))), end, 4);
  return cptr_clone((mem));
}

function isOverrun(mem: any | null): number {
  let guard = cptr_struct_overlay(GuardBytes, mem); /* &ref */
  let memAsChar = cptr_clone((mem)); /* &ref */
  (() => { const __t = guard; guard = __struct_ptr_advance(guard, -1); return __t; })();
  return (((((__struct_ptr_at(guard, 0)).guard_space) >>> 0) != ((0) >>> 0) || strcmp(cptr_offset(memAsChar, ((((__struct_ptr_at(guard, 0)).size) >>> 0))), cptr_clone(end)) != 0) ? 1 : 0);
}

function release_memory(mem: any | null): void {
  let guard = cptr_struct_overlay(GuardBytes, mem); /* &ref */
  (() => { const __t = guard; guard = __struct_ptr_advance(guard, -1); return __t; })();
  malloc_count--;
  free(guard);
...
export function unity_free(mem: any | null): void {
  let overrun = 0;
  if (cptr_eq(mem, (null))) {
    return;
  }
  ...
  release_memory(mem);
  if (overrun) {
    UNITY_TEST_FAIL("Buffer overrun detected during free()");
  }
}

export function unity_calloc(num: number, size: number): any | null {
  let mem = unity_malloc(((num) >>> 0) * ((size) >>> 0)); /* &ref */
  if (cptr_eq(mem, (null))) {
    return cptr_clone((null));
  }
  memset(mem, 0, ((num) >>> 0) * ((size) >>> 0));
  return cptr_clone(mem);
}

export function unity_realloc(oldMem: any | null, size: number): any | null {
  let guard = cptr_struct_overlay(GuardBytes, oldMem); /* &ref */
  let newMem = null;
  if (cptr_eq(oldMem, (null))) {
    return cptr_clone(unity_malloc(((size) >>> 0)));
  }
  (() => { const __t = guard; guard = __struct_ptr_advance(guard, -1); return __t; })();
  if (isOverrun(oldMem)) {
    release_memory(oldMem);
    UNITY_TEST_FAIL("Buffer overrun detected during realloc()");
  }
  if (((size) >>> 0) == ((0) >>> 0)) {
    release_memory(oldMem);
    return cptr_clone((null));
  }
  if ((((__struct_ptr_at(guard, 0)).size) >>> 0) >= ((size) >>> 0)) {
    return cptr_clone(oldMem);
  }
  newMem = unity_malloc(((size) >>> 0));
  if (cptr_eq(newMem, (null))) {
    return cptr_clone((null));
  }
  memcpy(newMem, oldMem, (((__struct_ptr_at(guard, 0)).size) >>> 0));
  release_memory(oldMem);
  return cptr_clone(newMem);
}
...
(PointerPair as any).__fieldTypes = ["int64","int64"];
(PointerPair as any).__fieldNames = ["pointer","old_value"];
(PointerPair as any).__fieldOffsets = [0,8];

let pointer_store = Array.from({length: 5}, () => new PointerPair());
let pointer_index = 0;
export function UnityPointer_Init(): void {
  pointer_index = 0;
}

export function UnityPointer_Set(pointer: { value: any | null }, newValue: any | null, line: number): void {
  if (pointer_index >= 5) {
    UNITY_TEST_FAIL(undefined /* recovery */, "Too many pointers set");
  } else {
    pointer_store[pointer_index].pointer = pointer;
    pointer_store[pointer_index].old_value = pointer.value;
    pointer.value = newValue;
    pointer_index++;
  }
}

export function UnityPointer_UndoAllSets(): void {
  while (pointer_index > 0) {
    pointer_index--;
    (pointer_store[pointer_index].pointer).value = pointer_store[pointer_index].old_value;
  }
}

export function UnityGetCommandLineOptions(argc: number, argv: { value: string }): number {
  let i = 0;
  UnityFixture.Verbose = 0;
  UnityFixture.GroupFilter = null;
  UnityFixture.NameFilter = null;
  UnityFixture.RepeatCount = ((1) >>> 0);
  if (argc == 1) {
    return 0;
  }
  for (i = 1; i < argc; ) {
    if (strcmp(cptr_clone(cptr_read_ptr(argv, i)), "-v") == 0) {
      UnityFixture.Verbose = 1;
      i++;
    } else {
      if (strcmp(cptr_clone(cptr_read_ptr(argv, i)), "-g") == 0) {
        i++;
        if (i >= argc) {
          return 1;
        }
        UnityFixture.GroupFilter = cptr_read_ptr(argv, i);
        i++;
      } else {
        if (strcmp(cptr_clone(cptr_read_ptr(argv, i)), "-n") == 0) {
          i++;
          if (i >= argc) {
            return 1;
          }
          UnityFixture.NameFilter = cptr_read_ptr(argv, i);
          i++;
        } else {
          if (strcmp(cptr_clone(cptr_read_ptr(argv, i)), "-r") == 0) {
            UnityFixture.RepeatCount = ((2) >>> 0);
            i++;
            if (i < argc) {
              if ((((((cptr_read_ptr(argv, i)).buf[(cptr_read_ptr(argv, i)).off]) << 24 >> 24) >= 48 && (((cptr_read_ptr(argv, i)).buf[(cptr_read_ptr(argv, i)).off]) << 24 >> 24) <= 57) ? 1 : 0)) {
                let digit = ((0) >>> 0);
                UnityFixture.RepeatCount = ((0) >>> 0);
                while (((((((typeof (cptr_read_ptr(argv, i)) === 'string') ? (cptr_read_ptr(argv, i)).charCodeAt(((digit) >>> 0)) : (cptr_read_ptr(argv, i)).buf[((cptr_read_ptr(argv, i)).off ?? 0) + (((digit) >>> 0))])) << 24 >> 24) >= 48 && ((((typeof (cptr_read_ptr(argv, i)) === 'string') ? (cptr_read_ptr(argv, i)).charCodeAt(((digit) >>> 0)) : (cptr_read_ptr(argv, i)).buf[((cptr_read_ptr(argv, i)).off ?? 0) + (((digit) >>> 0))])) << 24 >> 24) <= 57) ? 1 : 0)) {
                  UnityFixture.RepeatCount = (Math.imul(UnityFixture.RepeatCount, ((10) >>> 0)) >>> 0);
                  UnityFixture.RepeatCount = u32(UnityFixture.RepeatCount + u32(((Math.trunc(+(((((typeof (cptr_read_ptr(argv, i)) === 'string') ? (cptr_read_ptr(argv, i)).charCodeAt((() => { const _t = digit; digit = u32(digit + 1); return _t; })()) : (cptr_read_ptr(argv, i)).buf[((cptr_read_ptr(argv, i)).off ?? 0) + ((() => { const _t = digit; digit = u32(digit + 1); return _t; })())])) << 24 >> 24)))) >>> 0) - ((48) >>> 0)));
                }
                i++;
              }
            }
...
export function UnityConcludeFixtureTest(): void {
  if (undefined /* recovery */) {
    UNITY_PRINT_EOL();
  } else {
    if (undefined /* recovery */) {

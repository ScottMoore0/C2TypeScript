...
let buffer = null;
let spy_enable = 0;
export function UnityOutputCharSpy_Create(s: number): void {
  size = ((s > 0) ? s : 0);
  count = 0;
  spy_enable = 0;
  buffer = cptr_clone(malloc(((Math.trunc(+(size))) >>> 0)));
  TEST_ASSERT_NOT_NULL_MESSAGE(cptr_clone(buffer), "Internal malloc failed in Spy Create():tests/real-world/cjson/tests/unity/extras/fixture/test/unity_output_Spy.c");
  memset(buffer, 0, ((Math.trunc(+(size))) >>> 0));
}

export function UnityOutputCharSpy_Destroy(): void {
  size = 0;
  free(buffer);
}

export function UnityOutputCharSpy_OutputChar(c: number): void {
  if (spy_enable) {
    if (count < (i32(size - 1))) {
      buffer.buf[(buffer.off ?? 0) + count++] = (((Math.trunc(+(c))) << 24 >> 24)) << 24 >> 24;
    }
  } else {
    putchar(c);
  }
...
export function UnityOutputCharSpy_Get(): string {
  return cptr_clone(buffer);
}

export function UnityOutputCharSpy_Enable(enable: number): void {
  spy_enable = enable;
}

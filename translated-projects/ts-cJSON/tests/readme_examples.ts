...
let _static_monitor_without_hd_2: string = "{\n\t\t\"name\": \"lame monitor\",\n\t\t\"resolutions\":\t[{\n\t\t\t\"width\":\t640,\n\t\t\t\"height\":\t480\n\t\t}]\n}";
...
const json = cptr_clone("{\n\t\"name\":\t\"Awesome 4K\",\n\t\"resolutions\":\t[{\n\t\t\t\"width\":\t1280,\n\t\t\t\"height\":\t720\n\t\t}, {\n\t\t\t\"width\":\t1920,\n\t\t\t\"height\":\t1080\n\t\t}, {\n\t\t\t\"width\":\t3840,\n\t\t\t\"height\":\t2160\n\t\t}]\n}"); /* &ref */
function create_monitor(): string {
  let resolution_numbers: number[][] = Array.from({length: 3}, () => Array.from({length: 2}, () => 0));
  let string: string | null = null;
  let name: cJSON | null = null;
  let resolutions: cJSON | null = null;
  let resolution: cJSON | null = null;
  let width: cJSON | null = null;
  let height: cJSON | null = null;
  let index: number = 0;
  let monitor: cJSON | null = null;
  let _state = 0;
  _sm: while (true) {
    switch (_state) {
    case 0:
      resolution_numbers = [[((1280) >>> 0), ((720) >>> 0)], [((1920) >>> 0), ((1080) >>> 0)], [((3840) >>> 0), ((2160) >>> 0)]];
      string = null; /* &ref */
      name = null; /* &ref */
      resolutions = null; /* &ref */
      resolution = null; /* &ref */
      width = null; /* &ref */
      height = null; /* &ref */
      index = ((0) >>> 0);
      monitor = cJSON_CreateObject(); /* &ref */
      if (monitor == (null)) {
        _state = 1; continue _sm; /* goto end */
      }
      ...
      for (index = ((0) >>> 0); ((index) >>> 0) < (__safe_div(16, (2 * 4))); (index = u32(index + 1))) {
        resolution = cJSON_CreateObject();
        if (resolution == (null)) {
          _state = 1; continue _sm; /* goto end */
        }
        cJSON_AddItemToArray(resolutions, resolution);
        width = cJSON_CreateNumber(((resolution_numbers[((index) >>> 0)][0]) >>> 0));
        if (width == (null)) {
          _state = 1; continue _sm; /* goto end */
        }
      ...
      if (cptr_eq(string, (null))) {
        _fprintf((__acrt_iob_func(((2) >>> 0))), "Failed to print monitor.\n");
      }
    case 1: /* end */
...
function create_monitor_with_helpers(): string {
  let resolution_numbers: number[][] = Array.from({length: 3}, () => Array.from({length: 2}, () => 0));
  let string: string | null = null;
  let resolutions: cJSON | null = null;
  let index: number = 0;
  ...
  _sm: while (true) {
    ...
    case 0:
      ...
      for (index = ((0) >>> 0); ((index) >>> 0) < (__safe_div(16, (2 * 4))); (index = u32(index + 1))) {
        resolution = cJSON_CreateObject(); /* &ref */
        if (cJSON_AddNumberToObject(resolution, "width", ((resolution_numbers[((index) >>> 0)][0]) >>> 0)) == (null)) {
          _state = 1; continue _sm; /* goto end */
        }
        if (cJSON_AddNumberToObject(resolution, "height", ((resolution_numbers[((index) >>> 0)][1]) >>> 0)) == (null)) {
          _state = 1; continue _sm; /* goto end */
        }
        cJSON_AddItemToArray(resolutions, resolution);
      }
...
function supports_full_hd(monitor: string): number {
  let resolution: cJSON | null = null;
  let resolutions: cJSON | null = null;
  let name: cJSON | null = null;
  let status: number = 0;
  let monitor_json: cJSON | null = null;
  let error_ptr: string | null = null;
  let width: cJSON | null = null;
  let height: cJSON | null = null;
  let _state = 0;
  _sm: while (true) {
    ...
    case 0:
      ...
      name = null; /* &ref */
      status = 0;
      monitor_json = cJSON_Parse(cptr_clone(monitor)); /* &ref */
      if (monitor_json == (null)) {
        error_ptr = cJSON_GetErrorPtr(); /* &ref */
        if (!cptr_eq(error_ptr, (null))) {
          _fprintf((__acrt_iob_func(((2) >>> 0))), "Error before: %s\n", cptr_clone(error_ptr));
        }
        status = 0;
        _state = 1; continue _sm; /* goto end */
      }
      name = cJSON_GetObjectItemCaseSensitive(monitor_json, "name");
      if (((cJSON_IsString(name) && (!cptr_eq((__struct_ptr_at(name, 0)).valuestring, (null)))) ? 1 : 0)) {
        printf("Checking monitor \"%s\"\n", cptr_clone((__struct_ptr_at(name, 0)).valuestring));
      }
      resolutions = cJSON_GetObjectItemCaseSensitive(monitor_json, "resolutions");
      for (resolution = ((resolutions != (null)) ? (__struct_ptr_at((resolutions), 0)).child : null); resolution != (null); resolution = (__struct_ptr_at(resolution, 0)).next) {
        width = cJSON_GetObjectItemCaseSensitive(resolution, "width"); /* &ref */
        height = cJSON_GetObjectItemCaseSensitive(resolution, "height"); /* &ref */
        if (((!cJSON_IsNumber(width) || !cJSON_IsNumber(height)) ? 1 : 0)) {
          status = 0;
          _state = 1; continue _sm; /* goto end */
        }
        if (((compare_double((__struct_ptr_at(width, 0)).valuedouble, 1920) && compare_double((__struct_ptr_at(height, 0)).valuedouble, 1080)) ? 1 : 0)) {
          status = 1;
          _state = 1; continue _sm; /* goto end */
        }
      }
...
function create_monitor_should_create_a_monitor(): void {
  let monitor = create_monitor(); /* &ref */
  UnityAssertEqualString(cptr_clone((((monitor)))), cptr_clone((((json)))), null, __u64(__as_bigint((221))));
  free(monitor);
}

function create_monitor_with_helpers_should_create_a_monitor(): void {
  let monitor = create_monitor_with_helpers(); /* &ref */
  UnityAssertEqualString(cptr_clone((((json)))), cptr_clone((((monitor)))), null, __u64(__as_bigint((230))));
  free(monitor);
}

function supports_full_hd_should_check_for_full_hd_support(): void {
  if ((supports_full_hd(cptr_clone(json)))) {
  } else {
    UnityFail(((" Expression Evaluated To FALSE")), __u64(__as_bigint((__u64(__as_bigint((245)))))));
  }
  if (!(supports_full_hd(cptr_clone(_static_monitor_without_hd_2)))) {
  } else {
    UnityFail(((" Expected FALSE Was TRUE")), __u64(__as_bigint((__u64(__as_bigint((246)))))));
  }
}

export function main(): number {
  UnityBegin("tests/real-world/cjson/tests/readme_examples.c");
  UnityDefaultTestRun((create_monitor_should_create_a_monitor), "create_monitor_should_create_a_monitor", (253));
  UnityDefaultTestRun((create_monitor_with_helpers_should_create_a_monitor), "create_monitor_with_helpers_should_create_a_monitor", (254));
  UnityDefaultTestRun((supports_full_hd_should_check_for_full_hd_support), "supports_full_hd_should_check_for_full_hd_support", (255));
  return UnityEnd();
}

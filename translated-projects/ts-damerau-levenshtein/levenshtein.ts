...
export function levenshtein(a: any, b: any): number {
  if (typeof a === 'string') a = cptr_from_string(a);
  if (typeof b === 'string') b = cptr_from_string(b);

  let a_len = strlen(cptr_clone(a));
  let b_len = strlen(cptr_clone(b));
  let a_idx = 0;
  let b_idx = 0;
  let a_dist = 0;
  let b_dist = 0;
  let result = 0;
  let cache = calloc(a_len, 4); /* &ref */
  let code = 0;
  if ((cptr_eq(a, b) ? 1 : 0)) {
    return 0;
  }
  ...
  while ((a_idx < a_len ? 1 : 0)) {
    cptr_write_int32(cache, a_idx, i32(a_idx + 1));
    ++a_idx;
  }
  while ((b_idx < b_len ? 1 : 0)) {
    code = (((b.buf[(b.off ?? 0) + b_idx]) << 24 >> 24)) << 24 >> 24;
    result = a_dist = b_idx++;
    a_idx = -1;
    while ((++a_idx < a_len ? 1 : 0)) {
      b_dist = (((((code) << 24 >> 24) == ((a.buf[(a.off ?? 0) + a_idx]) << 24 >> 24) ? 1 : 0)) ? (a_dist) : (i32(a_dist + 1)));
      a_dist = cptr_read_int32(cache, a_idx);
      cptr_write_int32(cache, a_idx, result = (((a_dist > result ? 1 : 0)) ? ((((b_dist > result ? 1 : 0)) ? (i32(result + 1)) : (b_dist))) : ((((b_dist > a_dist ? 1 : 0)) ? (i32(a_dist + 1)) : (b_dist)))));
    }
  }
  free(cache);
  return result;
...
export function damerau_levenshtein(a: any, b: any): number {
  if (typeof a === 'string') a = cptr_from_string(a);
  if (typeof b === 'string') b = cptr_from_string(b);

  let a_len = strlen(cptr_clone(a));
  let b_len = strlen(cptr_clone(b));
  let v = calloc(i32(b_len + 1), 4); /* &ref */
  let i = 0;
  let j = 0;
  let current = 0;
  let next = 0;
  let cost = 0;
  while ((((((a_len > 0 ? 1 : 0) && (b_len > 0 ? 1 : 0)) ? 1 : 0) && (((a.buf[a.off]) << 24 >> 24) == ((b.buf[b.off]) << 24 >> 24) ? 1 : 0)) ? 1 : 0)) {
    a.off++;
    b.off++;
    a_len--;
    b_len--;
  ...
  for (j = 0; (j < i32(b_len + 1) ? 1 : 0); j++) {
    cptr_write_int32(v, j, j);
  }
  for (i = 0; (i < a_len ? 1 : 0); i++) {
    current = i32(i + 1);
    for (j = 0; (j < b_len ? 1 : 0); j++) {
      cost = (!((((((a.buf[(a.off ?? 0) + i]) << 24 >> 24) == ((b.buf[(b.off ?? 0) + j]) << 24 >> 24) ? 1 : 0) || (((((((i && j) ? 1 : 0) && (((a.buf[(a.off ?? 0) + i32(i - 1)]) << 24 >> 24) == ((b.buf[(b.off ?? 0) + j]) << 24 >> 24) ? 1 : 0)) ? 1 : 0) && (((a.buf[(a.off ?? 0) + i]) << 24 >> 24) == ((b.buf[(b.off ?? 0) + i32(j - 1)]) << 24 >> 24) ? 1 : 0)) ? 1 : 0))) ? 1 : 0)) ? 1 : 0);
      next = ((((((((((i32(cptr_read_int32(v, i32(j + 1)) + 1)) < (i32(current + 1)) ? 1 : 0)) ? (i32(cptr_read_int32(v, i32(j + 1)) + 1)) : (i32(current + 1))))) < (i32(cptr_read_int32(v, j) + cost)) ? 1 : 0)) ? ((((((i32(cptr_read_int32(v, i32(j + 1)) + 1)) < (i32(current + 1)) ? 1 : 0)) ? (i32(cptr_read_int32(v, i32(j + 1)) + 1)) : (i32(current + 1))))) : (i32(cptr_read_int32(v, j) + cost))));
      cptr_write_int32(v, j, current);
      current = next;
    }
    cptr_write_int32(v, b_len, next);
  }
  free(v);
  return next;
}

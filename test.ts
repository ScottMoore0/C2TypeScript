import { cJSON_AddFalseToObject, cJSON_AddItemToArray, cJSON_AddItemToObject, cJSON_AddNumberToObject, cJSON_AddStringToObject, cJSON_CreateArray, cJSON_CreateIntArray, cJSON_CreateObject, cJSON_CreateString, cJSON_CreateStringArray, cJSON_Delete, cJSON_Print, cJSON_PrintPreallocated, cJSON_Version, error } from './cJSON.js';
...
(record as any).__fieldTypes = ["int64","double","double","int64","int64","int64","int64","int64"];
(record as any).__fieldNames = ["precision","lat","lon","address","city","state","zip","country"];
(record as any).__fieldOffsets = [0,8,16,24,32,40,48,56];

function print_preallocated(root: cJSON | null): number {
  let out = null; /* &ref */
  let buf = null; /* &ref */
  let buf_fail = null; /* &ref */
  let len = ((0) >>> 0);
  let len_fail = ((0) >>> 0);
  out = cJSON_Print(root);
  len = strlen(cptr_clone(out)) + 5;
  buf = (malloc(((len) >>> 0)));
  if (cptr_eq(buf, (null))) {
    printf("Failed to allocate memory.\n");
    exit(1);
  }
  len_fail = strlen(cptr_clone(out));
  buf_fail = (malloc(((len_fail) >>> 0)));
  if (cptr_eq(buf_fail, (null))) {
    printf("Failed to allocate memory.\n");
    exit(1);
  }
  if (!cJSON_PrintPreallocated(root, cptr_clone(buf), (Number(BigInt.asIntN(32, __as_bigint(((len) >>> 0)))) | 0), 1)) {
    printf("cJSON_PrintPreallocated failed!\n");
    if (strcmp(cptr_clone(out), cptr_clone(buf)) != 0) {
  ...
  printf("%s\n", cptr_clone(buf));
  if (cJSON_PrintPreallocated(root, cptr_clone(buf_fail), (Number(BigInt.asIntN(32, __as_bigint(((len_fail) >>> 0)))) | 0), 1)) {
...
function create_objects(): void {
  ...
  let fld = null; /* &ref */
  let i = 0;
  let strings = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]; /* &ref */
  let numbers = [[0, -1, 0], [1, 0, 0], [0, 0, 1]];
  let ids = [116, 943, 234, 38793];
  let fields = [Object.assign(new record(), { precision: "zip", lat: 37.766800000000003, lon: -122.3959, address: "", city: "SAN FRANCISCO", state: "CA", zip: "94107", country: "US" }), Object.assign(new record(), { precision: "zip", lat: 37.371991000000001, lon: -122.026, address: "", city: "SUNNYVALE", state: "CA", zip: "94085", country: "US" })];
  ...
  cJSON_Delete(root);
  root = cJSON_CreateStringArray(cptr_clone(strings), 7);
  if (print_preallocated(root) != 0) {
    cJSON_Delete(root);
    exit(1);
  }
  ...
  for (i = 0; i < 2; i++) {
    ...
    cJSON_AddStringToObject(fld, "precision", cptr_clone(fields[i].precision));
    cJSON_AddNumberToObject(fld, "Latitude", fields[i].lat);
    cJSON_AddNumberToObject(fld, "Longitude", fields[i].lon);
    cJSON_AddStringToObject(fld, "Address", cptr_clone(fields[i].address));
    cJSON_AddStringToObject(fld, "City", cptr_clone(fields[i].city));
    cJSON_AddStringToObject(fld, "State", cptr_clone(fields[i].state));
    cJSON_AddStringToObject(fld, "Zip", cptr_clone(fields[i].zip));
    cJSON_AddStringToObject(fld, "Country", cptr_clone(fields[i].country));
  }
  if (print_preallocated(root) != 0) {
    cJSON_Delete(root);
    exit(1);
  ...
  cJSON_Delete(root);
  root = cJSON_CreateObject();
  cJSON_AddNumberToObject(root, "number", 1 / zero);
  if (print_preallocated(root) != 0) {
    cJSON_Delete(root);
    exit(1);
  }

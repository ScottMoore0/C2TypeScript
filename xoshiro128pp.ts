...
let _static_JUMP_0: number[] = [2271477771, 4114797267, ((1872770499) >>> 0), ((2012404571) >>> 0)];
let _static_LONG_JUMP_1: number[] = [3039008046, ((191826335) >>> 0), 3438649583, ((475530850) >>> 0)];

export function rotl(x: number, k: number): number {
  return (((((x) >>> 0) << k) >>> 0) | ((((x) >>> 0) >>> (i32(32 - k))) >>> 0)) >>> 0;
}

export let s = new Array(4).fill(0);
export function xoshiro128pp_next(): number {
  let result = u32(rotl(u32(((s[0]) >>> 0) + ((s[3]) >>> 0)), 7) + ((s[0]) >>> 0));
  let t = (((s[1]) >>> 0) << 9) >>> 0;
  s[2] = (s[2] ^ ((s[0]) >>> 0)) >>> 0;
  s[3] = (s[3] ^ ((s[1]) >>> 0)) >>> 0;
  s[1] = (s[1] ^ ((s[2]) >>> 0)) >>> 0;
...
export function xoshiro128pp_seed(a: number, b: number, c: number, d: number): void {
  s[0] = ((a) >>> 0);
  s[1] = ((b) >>> 0);
  s[2] = ((c) >>> 0);
  s[3] = ((d) >>> 0);
...
export function xoshiro128pp_get_state(out: any | null): void {
  cptr_write_uint32(out, 0, ((s[0]) >>> 0));
  cptr_write_uint32(out, 1, ((s[1]) >>> 0));
  cptr_write_uint32(out, 2, ((s[2]) >>> 0));
  cptr_write_uint32(out, 3, ((s[3]) >>> 0));
}

export function xoshiro128pp_set_state(_in: any | null): void {
  s[0] = ((cptr_read_uint32(_in, 0)) >>> 0);
  s[1] = ((cptr_read_uint32(_in, 1)) >>> 0);
  s[2] = ((cptr_read_uint32(_in, 2)) >>> 0);
  s[3] = ((cptr_read_uint32(_in, 3)) >>> 0);
}

export function xoshiro128pp_jump(): void {
  let s0 = ((0) >>> 0);
  let s1 = ((0) >>> 0);
  let s2 = ((0) >>> 0);
  let s3 = ((0) >>> 0);
  for (let i: number = 0; (i < (Math.trunc(+((__safe_div(16, 4)))) | 0) ? 1 : 0); i++) {
    for (let b: number = 0; (b < 32 ? 1 : 0); b++) {
      if ((((_static_JUMP_0[i]) >>> 0) & (((1) << b) >>> 0)) >>> 0) {
        s0 = (s0 ^ ((s[0]) >>> 0)) >>> 0;
        s1 = (s1 ^ ((s[1]) >>> 0)) >>> 0;
        s2 = (s2 ^ ((s[2]) >>> 0)) >>> 0;
        s3 = (s3 ^ ((s[3]) >>> 0)) >>> 0;
...
export function xoshiro128pp_long_jump(): void {
  let s0 = ((0) >>> 0);
  let s1 = ((0) >>> 0);
  let s2 = ((0) >>> 0);
  let s3 = ((0) >>> 0);
  for (let i: number = 0; (i < (Math.trunc(+((__safe_div(16, 4)))) | 0) ? 1 : 0); i++) {
    for (let b: number = 0; (b < 32 ? 1 : 0); b++) {
      if ((((_static_LONG_JUMP_1[i]) >>> 0) & (((1) << b) >>> 0)) >>> 0) {
        s0 = (s0 ^ ((s[0]) >>> 0)) >>> 0;
        s1 = (s1 ^ ((s[1]) >>> 0)) >>> 0;
        s2 = (s2 ^ ((s[2]) >>> 0)) >>> 0;
        s3 = (s3 ^ ((s[3]) >>> 0)) >>> 0;

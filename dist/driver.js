...
function assert(expr, msg) { if (!expr) {
    const m = 'Assertion failed' + (msg ? ': ' + msg : '');
    process.stderr.write(m + '\n');
    throw new Error(m);
} }
function _wassert(_expr, _file, _line) { const m = 'Assertion failed: ' + String(_expr); process.stderr.write(m + '\n'); throw new Error(m); }
...
function prev(it, n = 1) { if (it == null)
    return null; if (it && it.__arr !== undefined)
    return __cpp_iter(it.__arr, (it.__pos ?? 0) - n); if (typeof it === 'number')
    return it - n; return it; }
function next(it, n = 1) { if (it == null)
    return null; if (it && it.__arr !== undefined)
    return __cpp_iter(it.__arr, (it.__pos ?? 0) + n); if (typeof it === 'number')
    return it + n; return it; }
...
function __safe_mod_i64(a, b) { if (b === 0n)
    throw new Error('Division by zero'); return a % b; }
const __cpp2ts_objId_map = new WeakMap();
const __cpp2ts_objId_inverse = new Map();
let __cpp2ts_objId_next = 64;
function __cpp2ts_objId(o) { if (o == null || typeof o !== 'object')
    return 0; let id = __cpp2ts_objId_map.get(o); if (id === undefined) {
    id = __cpp2ts_objId_next;
    __cpp2ts_objId_next += 64;
    __cpp2ts_objId_map.set(o, id);
    __cpp2ts_objId_inverse.set(id, o);
} return id; }
const __cpp2ts_cptrInt_byBuf = new WeakMap();
const __cpp2ts_cptrInt_inverse = new Map();
let __cpp2ts_cptrInt_next = -64;
function __cpp2ts_ptr_to_intptr(p) {
    if (typeof p === 'string')
        p = cptr_from_string(p);
    if (p == null)
        return 0;
    if (p && p.buf && typeof p.off !== 'undefined') {
        let m = __cpp2ts_cptrInt_byBuf.get(p.buf);
        if (!m) {
            m = new Map();
            __cpp2ts_cptrInt_byBuf.set(p.buf, m);
        }
        const off = p.off ?? 0;
        let id = m.get(off);
        if (id === undefined) {
            id = __cpp2ts_cptrInt_next;
            __cpp2ts_cptrInt_next -= 64;
            m.set(off, id);
            __cpp2ts_cptrInt_inverse.set(id, { buf: p.buf, off });
        }
        return id;
    }
    return __cpp2ts_objId(p);
}
function __cpp2ts_intptr_to_ptr(i) { if (i === 0 || i === 0n || i == null)
    return null; const n = typeof i === 'bigint' ? Number(i) : i; if (__cpp2ts_cptrInt_inverse.has(n))
    return __cpp2ts_cptrInt_inverse.get(n); if (__cpp2ts_objId_inverse.has(n))
    return __cpp2ts_objId_inverse.get(n); return n; }
...
int_node.__fieldTypes = ["int32", "int64", "int64", "int64", "int64", "int32"];
int_node.__fieldNames = ["v", "next", "prev", "left", "right", "color"];
int_node.__fieldOffsets = [0, 8, 16, 24, 32, 40];
// BRIDGE: struct-as-class — C17 §6.7.2.1 / C++20 [class]
...
str_node_list.__fieldTypes = ["int64", "int64"];
str_node_list.__fieldNames = ["key", "next"];
str_node_list.__fieldOffsets = [0, 8];
// BRIDGE: struct-as-class — C17 §6.7.2.1 / C++20 [class]
...
int_node_sl.__fieldTypes = ["int32", "int64"];
int_node_sl.__fieldNames = ["v", "next"];
int_node_sl.__fieldOffsets = [0, 8];
// BRIDGE: struct-as-class — C17 §6.7.2.1 / C++20 [class]
...
int_node_rb.__fieldTypes = ["int32", "int64", "int64", "int32"];
int_node_rb.__fieldNames = ["v", "left", "right", "color"];
int_node_rb.__fieldOffsets = [0, 8, 16, 24];
// BRIDGE: struct-as-class — C17 §6.7.2.1 / C++20 [class]
...
int_node_dl.__fieldTypes = ["int32", "int64", "int64"];
int_node_dl.__fieldNames = ["v", "next", "prev"];
int_node_dl.__fieldOffsets = [0, 8, 16];
// BRIDGE: struct-as-class — C17 §6.7.2.1 / C++20 [class]
...
intkv_node.__fieldTypes = ["int32", "int32", "int64"];
intkv_node.__fieldNames = ["k", "v", "next_in_chain"];
intkv_node.__fieldOffsets = [0, 4, 8];
function str_cmp_chars(a, b) {
    if (typeof a === 'string')
        a = cptr_from_string(a);
    if (typeof b === 'string')
        b = cptr_from_string(b);
...
sglib_int_node_iterator.__fieldTypes = ["int64", "int64", "int32", "int64"];
sglib_int_node_iterator.__fieldNames = ["currentelem", "nextelem", "subcomparator", "equalto"];
sglib_int_node_iterator.__fieldOffsets = [0, 8, 16, 24];
export function sglib_int_node_is_member(list, elem) {
    let result = 0;
    {
        let _p_ = null;
...
export function sglib_int_node_find_member(list, elem) {
    let result = null;
    {
        let _p_ = null;
...
export function sglib_int_node_add_if_not_member(list, elem, member) {
    {
        let _p_ = null;
...
export function sglib_int_node_add(list, elem) {
    {
        (__struct_ptr_at((elem), 0)).next = (list.value);
        list.value = (elem);
    }
}
export function sglib_int_node_concat(first, second) {
    {
        if ((first.value) == (null)) {
            first.value = (second);
        }
...
export function sglib_int_node_delete(list, elem) {
    {
        let _p_ = null;
        for (_p_ = list; ((_p_.value != (null) && _p_.value != (elem)) ? 1 : 0); _p_ = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "int_node", "next", 8))((_p_.value))) {
        }
        (((((!!(((_p_.value != (null) && !cptr_eq("element is not member of the container, use DELETE_IF_MEMBER instead", (null))) ? 1 : 0))) || ((() => { _wassert("*_p_!=null && \"element is not member of the container, use DELETE_IF_MEMBER instead\"!=null", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((105)))) >>> 0)); return 0; })())) ? 1 : 0)));
        _p_.value = (_p_.value).next;
    }
}
export function sglib_int_node_delete_if_member(list, elem, member) {
    {
        let _p_ = null;
        for (_p_ = list; ((_p_.value != (null) && (i32(((_p_.value)).v - (__struct_ptr_at(((elem)), 0)).v)) != 0) ? 1 : 0); _p_ = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "int_node", "next", 8))((_p_.value))) {
        }
...
export function sglib_int_node_sort(list) {
    {
        let _r_ = null;
        let _a_ = null;
        let _b_ = null;
        ...
        for (_n_ = 1; _contFlag_; _n_ = i32(_n_ + _n_)) {
            ...
            while (_todo_ != (null)) {
                ...
                while (_restail_.value != (null)) {
                    _restail_ = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "int_node", "next", 8))((_restail_.value));
                }
                _contFlag_ = 1;
            }
        }
...
export function sglib_int_node_len(list) {
    let res = 0;
    {
        let _ce_ = null;
        ((_ce_));
...
export function sglib_int_node_reverse(list) {
    {
        let _list_ = null;
        let _tmp_ = null;
        let _res_ = null;
...
export function sglib_int_node_it_init_on_equal(it, list, subcomparator, equalto) {
...
export function sglib_int_node_it_init(it, list) {
    return (sglib_int_node_it_init_on_equal(it, list, null, null));
}
export function sglib_int_node_it_current(it) {
    return ((__struct_ptr_at(it, 0)).currentelem);
}
export function sglib_int_node_it_next(it) {
    let ce = null;
    let eq = null;
    let scp = null;
...
sglib_str_node_list_iterator.__fieldTypes = ["int64", "int64", "int32", "int64"];
sglib_str_node_list_iterator.__fieldNames = ["currentelem", "nextelem", "subcomparator", "equalto"];
sglib_str_node_list_iterator.__fieldOffsets = [0, 8, 16, 24];
export function sglib_str_node_list_is_member(list, elem) {
    let result = 0;
    {
        let _p_ = null;
        for (_p_ = (list); ((_p_ != (null) && _p_ != (elem)) ? 1 : 0); _p_ = (__struct_ptr_at(_p_, 0)).next) {
...
export function sglib_str_node_list_find_member(list, elem) {
    let result = null;
    {
        let _p_ = null;
...
export function sglib_str_node_list_add_if_not_member(list, elem, member) {
    {
        let _p_ = null;
...
export function sglib_str_node_list_add(list, elem) {
    {
        (__struct_ptr_at((elem), 0)).next = (list.value);
        list.value = (elem);
    }
}
export function sglib_str_node_list_concat(first, second) {
    {
        if ((first.value) == (null)) {
            first.value = (second);
        }
...
export function sglib_str_node_list_delete(list, elem) {
    {
        let _p_ = null;
        for (_p_ = list; ((_p_.value != (null) && _p_.value != (elem)) ? 1 : 0); _p_ = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "str_node_list", "next", 8))((_p_.value))) {
        }
        (((((!!(((_p_.value != (null) && !cptr_eq("element is not member of the container, use DELETE_IF_MEMBER instead", (null))) ? 1 : 0))) || ((() => { _wassert("*_p_!=null && \"element is not member of the container, use DELETE_IF_MEMBER instead\"!=null", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((109)))) >>> 0)); return 0; })())) ? 1 : 0)));
        _p_.value = (_p_.value).next;
    }
}
export function sglib_str_node_list_delete_if_member(list, elem, member) {
    {
        let _p_ = null;
        for (_p_ = list; ((_p_.value != (null) && (str_cmp_chars(cptr_clone(((_p_.value)).key), cptr_clone((__struct_ptr_at(((elem)), 0)).key))) != 0) ? 1 : 0); _p_ = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "str_node_list", "next", 8))((_p_.value))) {
        }
        member.value = _p_.value;
        if (_p_.value != (null)) {
            _p_.value = (_p_.value).next;
...
export function sglib_str_node_list_sort(list) {
    {
        let _r_ = null;
        let _a_ = null;
        let _b_ = null;
        ...
        for (_n_ = 1; _contFlag_; _n_ = i32(_n_ + _n_)) {
            ...
            while (_todo_ != (null)) {
                ...
                while (_restail_.value != (null)) {
                    _restail_ = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "str_node_list", "next", 8))((_restail_.value));
                }
                _contFlag_ = 1;
            }
        }
...
export function sglib_str_node_list_len(list) {
    let res = 0;
    {
        let _ce_ = null;
        ((_ce_));
...
export function sglib_str_node_list_reverse(list) {
    {
        let _list_ = null;
        let _tmp_ = null;
        let _res_ = null;
...
export function sglib_str_node_list_it_init_on_equal(it, list, subcomparator, equalto) {
    (__struct_ptr_at(it, 0)).subcomparator = subcomparator;
    (__struct_ptr_at(it, 0)).equalto = equalto;
    (__struct_ptr_at(it, 0)).nextelem = list;
    return (sglib_str_node_list_it_next(it));
}
export function sglib_str_node_list_it_init(it, list) {
    return (sglib_str_node_list_it_init_on_equal(it, list, null, null));
}
export function sglib_str_node_list_it_current(it) {
    return ((__struct_ptr_at(it, 0)).currentelem);
}
export function sglib_str_node_list_it_next(it) {
    let ce = null;
    let eq = null;
    let scp = null;
    ce = (__struct_ptr_at(it, 0)).nextelem;
...
sglib_int_node_dl_iterator.__fieldTypes = ["int64", "int64", "int64", "int32", "int64"];
sglib_int_node_dl_iterator.__fieldNames = ["currentelem", "prevelem", "nextelem", "subcomparator", "equalto"];
sglib_int_node_dl_iterator.__fieldOffsets = [0, 8, 16, 24, 32];
export function sglib_int_node_dl_add(list, elem) {
    {
        {
            {
                if ((list.value) == (null)) {
...
export function sglib_int_node_dl_add_after(list, elem) {
    {
        if ((list.value) == (null)) {
            {
                {
...
export function sglib_int_node_dl_add_before(list, elem) {
    {
        if ((list.value) == (null)) {
            {
                {
...
export function sglib_int_node_dl_add_if_not_member(list, elem, member) {
    {
        {
            {
                let _dlp_ = null;
...
export function sglib_int_node_dl_add_after_if_not_member(list, elem, member) {
    {
        {
            {
                let _dlp_ = null;
...
export function sglib_int_node_dl_add_before_if_not_member(list, elem, member) {
    {
        {
            {
                let _dlp_ = null;
...
export function sglib_int_node_dl_concat(first, second) {
    {
        if ((first.value) == (null)) {
            first.value = (second);
        }
...
export function sglib_int_node_dl_delete(list, elem) {
    {
        let _l_ = null;
        _l_ = (list.value);
        if (_l_ == (elem)) {
...
export function sglib_int_node_dl_delete_if_member(list, elem, member) {
    {
        let _dlp_ = null;
        for (_dlp_ = (list.value); ((_dlp_ != (null) && (i32((__struct_ptr_at((_dlp_), 0)).v - (__struct_ptr_at(((elem)), 0)).v)) != 0) ? 1 : 0); _dlp_ = (__struct_ptr_at(_dlp_, 0)).prev) {
        }
...
export function sglib_int_node_dl_is_member(list, elem) {
    let result = 0;
    {
        let _dlp_ = null;
        {
...
export function sglib_int_node_dl_find_member(list, elem) {
    let result = null;
    {
        let _dlp_ = null;
        {
...
export function sglib_int_node_dl_get_first(list) {
    let result = null;
    {
        let _dll_ = null;
        _dll_ = (list);
...
export function sglib_int_node_dl_get_last(list) {
    let result = null;
    {
        let _dll_ = null;
        _dll_ = (list);
...
export function sglib_int_node_dl_sort(list) {
    {
        let _dll_ = null;
        _dll_ = (list.value);
        if (_dll_ != (null)) {
            ...
            {
                {
                    ...
                    for (_n_ = 1; _contFlag_; _n_ = i32(_n_ + _n_)) {
                        ...
                        while (_todo_ != (null)) {
                            ...
                            while (_restail_.value != (null)) {
                                _restail_ = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "int_node_dl", "next", 8))((_restail_.value));
                            }
                            _contFlag_ = 1;
                        }
                    }
...
export function sglib_int_node_dl_len(list) {
    let res = 0;
    {
        let _dl_ = null;
        let _r1_ = 0;
...
export function sglib_int_node_dl_reverse(list) {
    {
        let _list_ = null;
        let _nlist_ = null;
        let _dlp_ = null;
...
export function sglib_int_node_dl_it_init_on_equal(it, list, subcomparator, equalto) {
    (__struct_ptr_at(it, 0)).subcomparator = subcomparator;
    (__struct_ptr_at(it, 0)).equalto = equalto;
...
export function sglib_int_node_dl_it_init(it, list) {
    return (sglib_int_node_dl_it_init_on_equal(it, list, null, null));
}
export function sglib_int_node_dl_it_current(it) {
    return ((__struct_ptr_at(it, 0)).currentelem);
}
export function sglib_int_node_dl_it_next(it) {
    let ce = null;
    let eq = null;
    let scp = null;
...
export class sglib_int_node_rb_iterator {
    currentelem;
    pass = cptr_create(128);
    path = new Array(128).fill(null);
    pathi;
    order;
    equalto;
    subcomparator;
...
sglib_int_node_rb_iterator.__fieldTypes = ["int64", "bytes", "bytes", "int16", "int16", "int64", "int32"];
sglib_int_node_rb_iterator.__fieldNames = ["currentelem", "pass", "path", "pathi", "order", "equalto", "subcomparator"];
sglib_int_node_rb_iterator.__fieldOffsets = [0, 8, 136, 1160, 1162, 1168, 1176];
function sglib___int_node_rb_fix_left_insertion_discrepancy(tree) {
    {
        let t = null;
        let tl = null;
        let a = null;
...
function sglib___int_node_rb_fix_right_insertion_discrepancy(tree) {
    {
        let t = null;
        let tl = null;
        let a = null;
...
function sglib___int_node_rb_fix_left_deletion_discrepancy(tree) {
    let res = 0;
    {
        let t = null;
        let a = null;
        ...
        ((ar));
        t = a = tree.value;
        (((((!!(t != (null))) || ((() => { _wassert("t!=null", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
        ar = (__struct_ptr_at(a, 0)).left;
        b = (__struct_ptr_at(t, 0)).right;
        if (b == (null)) {
            (((((!!(((__struct_ptr_at(t, 0)).color) == 1)) || ((() => { _wassert("SGLIB___GET_VALUE(t->color)==1", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
            {
                {
            ...
            if (((__struct_ptr_at(b, 0)).color) == 1) {
                if (br == (null)) {
                    ...
                    c = br;
                    (((((!!(((c != (null) && ((__struct_ptr_at(c, 0)).color) == 0) ? 1 : 0))) || ((() => { _wassert("c!=null && SGLIB___GET_VALUE(c->color)==0", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
                    cl = (__struct_ptr_at(c, 0)).right;
                    cr = (__struct_ptr_at(c, 0)).left;
                    if ((((((cl == (null) || ((__struct_ptr_at(cl, 0)).color) == 0) ? 1 : 0)) && (((cr == (null) || ((__struct_ptr_at(cr, 0)).color) == 0) ? 1 : 0))) ? 1 : 0)) {
                        ...
                        if (((cl != (null) && ((__struct_ptr_at(cl, 0)).color) == 1) ? 1 : 0)) {
                            ...
                            if (((cr != (null) && ((__struct_ptr_at(cr, 0)).color) == 1) ? 1 : 0)) {
                                (((((!!(((cl == (null) || ((__struct_ptr_at(cl, 0)).color) == 0) ? 1 : 0))) || ((() => { _wassert("cl==null || SGLIB___GET_VALUE(cl->color)==0", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
                                d = cr;
                                dl = (__struct_ptr_at(d, 0)).right;
                                dr = (__struct_ptr_at(d, 0)).left;
                                tree.value = d;
                                ...
                                (((((!!(0)) || ((() => { _wassert("0", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
                                res = 0;
                            }
                        }
                    }
                ...
                if ((((((bl == (null) || ((__struct_ptr_at(bl, 0)).color) == 0) ? 1 : 0)) && (((br == (null) || ((__struct_ptr_at(br, 0)).color) == 0) ? 1 : 0))) ? 1 : 0)) {
                    ...
                    if (((bl != (null) && ((__struct_ptr_at(bl, 0)).color) == 1) ? 1 : 0)) {
                        if (((br == (null) || ((__struct_ptr_at(br, 0)).color) == 0) ? 1 : 0)) {
                            ...
                            (((((!!(bl != (null))) || ((() => { _wassert("bl!=null", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
                            (((((!!(br != (null))) || ((() => { _wassert("br!=null", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
                            (((((!!(((__struct_ptr_at(bl, 0)).color) == 1)) || ((() => { _wassert("SGLIB___GET_VALUE(bl->color)==1", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
                            (((((!!(((__struct_ptr_at(br, 0)).color) == 1)) || ((() => { _wassert("SGLIB___GET_VALUE(br->color)==1", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
                            c = br;
                            cl = (__struct_ptr_at(c, 0)).right;
                            cr = (__struct_ptr_at(c, 0)).left;
                            tree.value = c;
                        ...
                        (((((!!(((br != (null) && ((__struct_ptr_at(br, 0)).color) == 1) ? 1 : 0))) || ((() => { _wassert("br!=null && SGLIB___GET_VALUE(br->color)==1", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
                        c = br;
                        cl = (__struct_ptr_at(c, 0)).right;
                        cr = (__struct_ptr_at(c, 0)).left;
                        tree.value = c;
...
function sglib___int_node_rb_fix_right_deletion_discrepancy(tree) {
    let res = 0;
    {
        let t = null;
        let a = null;
...
function sglib___int_node_rb_add_recursive(tree, elem) {
    let cmp = 0;
    let t = null;
    t = tree.value;
    if (t == (null)) {
        ...
        if (((cmp < 0 || (((cmp == 0 && ((__l, __r) => { const __lb = __l && __l.buf; const __rb = __r && __r.buf; if (__lb && __rb && __lb === __rb)
            return ((__l.off ?? 0) < (__r.off ?? 0)); if (__lb || __rb)
            return (__cpp2ts_ptr_to_intptr(__l) < __cpp2ts_ptr_to_intptr(__r)); return ((__l ?? 0) < (__r ?? 0)); })(elem, t)) ? 1 : 0))) ? 1 : 0)) {
...
function sglib___int_node_rb_delete_rightmost_leaf(tree, theLeaf) {
    let t = null;
    let res = 0;
    let deepDecreased = 0;
    t = tree.value;
...
export function sglib___int_node_rb_delete_recursive(tree, elem) {
    let t = null;
    let theLeaf = null;
    let cmp = 0;
    let res = 0;
    ...
    res = 0;
    if (t == (null)) {
        (((((!!(((0 && !cptr_eq("The element to delete not found in the tree,  use 'delete_if_member'", (null))) ? 1 : 0))) || ((() => { _wassert("0 && \"The element to delete not found in the tree,  use 'delete_if_member'\"!=null", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
    }
    else {
        cmp = (i32((__struct_ptr_at((elem), 0)).v - (__struct_ptr_at((t), 0)).v));
        if (((cmp < 0 || (((cmp == 0 && ((__l, __r) => { const __lb = __l && __l.buf; const __rb = __r && __r.buf; if (__lb && __rb && __lb === __rb)
...
export function sglib_int_node_rb_add(tree, elem) {
...
export function sglib_int_node_rb_delete(tree, elem) {
    sglib___int_node_rb_delete_recursive(tree, elem);
    if (tree.value != (null)) {
        ((tree.value).color) = (0);
    }
}
export function sglib_int_node_rb_find_member(t, elem) {
    let res = null;
    {
        let _s_ = null;
        let _c_ = 0;
...
export function sglib_int_node_rb_is_member(t, elem) {
    let cmp = 0;
    while (t != (null)) {
        cmp = (i32((__struct_ptr_at((elem), 0)).v - (__struct_ptr_at((t), 0)).v));
        if (((cmp < 0 || (((cmp == 0 && ((__l, __r) => { const __lb = __l && __l.buf; const __rb = __r && __r.buf; if (__lb && __rb && __lb === __rb)
            ...
            if (((cmp > 0 || (((cmp == 0 && ((__l, __r) => { const __lb = __l && __l.buf; const __rb = __r && __r.buf; if (__lb && __rb && __lb === __rb)
                ...
                (((((!!(t == elem)) || ((() => { _wassert("t == elem", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
                return (1);
            }
        }
    }
...
export function sglib_int_node_rb_delete_if_member(tree, elem, memb) {
...
export function sglib_int_node_rb_add_if_not_member(tree, elem, memb) {
    if ((memb.value = sglib_int_node_rb_find_member(tree.value, elem)) == (null)) {
...
export function sglib_int_node_rb_len(t) {
    let n = 0;
    let e = null;
    ((e));
    n = 0;
        ...
        {
            {
                ...
                while (_cn_ != (null)) {
                    while (_cn_ != (null)) {
                        ...
                        _pathi_++;
                        if (_pathi_ >= 128) {
                            (((((!!(((0 && "the binary_tree is too deep") ? 1 : 0))) || ((() => { _wassert("0 && \"the binary_tree is too deep\"", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
                        }
                    }
                    do {
                        _pathi_--;
...
export function sglib__int_node_rb_it_compute_current_elem(it) {
    let i = 0;
    let j = 0;
    let s = null;
    let eqt = null;
...
export function sglib__int_node_rb_it_init(it, tree, order, subcomparator, equalto) {
    let t = null;
    (((((!!(it != (null))) || ((() => { _wassert("it!=null", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
...
export function sglib_int_node_rb_it_init(it, tree) {
    return (sglib__int_node_rb_it_init(it, tree, 2, null, null));
}
export function sglib_int_node_rb_it_init_preorder(it, tree) {
    return (sglib__int_node_rb_it_init(it, tree, 0, null, null));
}
export function sglib_int_node_rb_it_init_inorder(it, tree) {
    return (sglib__int_node_rb_it_init(it, tree, 1, null, null));
}
export function sglib_int_node_rb_it_init_postorder(it, tree) {
    return (sglib__int_node_rb_it_init(it, tree, 2, null, null));
}
export function sglib_int_node_rb_it_init_on_equal(it, tree, subcomparator, equalto) {
    return (sglib__int_node_rb_it_init(it, tree, 1, subcomparator, equalto));
}
export function sglib_int_node_rb_it_current(it) {
    return ((__struct_ptr_at(it, 0)).currentelem);
}
export function sglib_int_node_rb_it_next(it) {
    sglib__int_node_rb_it_compute_current_elem(it);
    return ((__struct_ptr_at(it, 0)).currentelem);
}
function sglib___int_node_rb_consistency_check_recursive(t, pathdeep, cdeep) {
    if (t == (null)) {
        if (pathdeep.value < 0) {
            (() => { const __p = (pathdeep); const __v = (cdeep); if (__p && __p.buf) {
                cptr_write_int32(__p, 0, __v);
            }
            else if (__p) {
                __p.value = __v;
            } })();
            ...
            (((((!!(pathdeep.value == cdeep)) || ((() => { _wassert("*pathdeep == cdeep", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
        }
    }
    else {
        if ((__struct_ptr_at(t, 0)).left != (null)) {
            (((((!!((i32((__struct_ptr_at(((__struct_ptr_at(t, 0)).left), 0)).v - (__struct_ptr_at((t), 0)).v)) <= 0)) || ((() => { _wassert("INT_NODE_RB_CMP(t->left, t) <= 0", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
        }
        if ((__struct_ptr_at(t, 0)).right != (null)) {
            (((((!!((i32((__struct_ptr_at((t), 0)).v - (__struct_ptr_at(((__struct_ptr_at(t, 0)).right), 0)).v)) <= 0)) || ((() => { _wassert("INT_NODE_RB_CMP(t, t->right) <= 0", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
        }
        if (((__struct_ptr_at(t, 0)).color) == 1) {
            (((((!!((((__struct_ptr_at(t, 0)).left == (null) || ((__struct_ptr_at((__struct_ptr_at(t, 0)).left, 0)).color) == 0) ? 1 : 0))) || ((() => { _wassert("t->left == null || SGLIB___GET_VALUE(t->left->color)==0", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
            (((((!!((((__struct_ptr_at(t, 0)).right == (null) || ((__struct_ptr_at((__struct_ptr_at(t, 0)).right, 0)).color) == 0) ? 1 : 0))) || ((() => { _wassert("t->right == null || SGLIB___GET_VALUE(t->right->color)==0", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
...
export function sglib___int_node_rb_consistency_check(t) {
    let pathDeep_box = { value: 0 };
    (((((!!(((t == (null) || ((__struct_ptr_at(t, 0)).color) == 0) ? 1 : 0))) || ((() => { _wassert("t==null || SGLIB___GET_VALUE(t->color) == 0", "C:/Users/scomo/cpp-to-ts/cpp-to-ts/compiler/tests/real-world/sglib-full/src/driver.c", ((Math.trunc(+((117)))) >>> 0)); return 0; })())) ? 1 : 0)));
...
sglib_intkv_node_iterator.__fieldTypes = ["int64", "int64", "int32", "int64"];
sglib_intkv_node_iterator.__fieldNames = ["currentelem", "nextelem", "subcomparator", "equalto"];
sglib_intkv_node_iterator.__fieldOffsets = [0, 8, 16, 24];
export function sglib_intkv_node_is_member(list, elem) {
    let result = 0;
    {
        let _p_ = null;
...
export function sglib_intkv_node_find_member(list, elem) {
    let result = null;
    {
        let _p_ = null;
...
export function sglib_intkv_node_add_if_not_member(list, elem, member) {
    {
        let _p_ = null;
...
export function sglib_intkv_node_add(list, elem) {
    {
        (__struct_ptr_at((elem), 0)).next_in_chain = (list.value);
        list.value = (elem);
    }
}
export function sglib_intkv_node_concat(first, second) {
    {
        if ((first.value) == (null)) {
            first.value = (second);
        }
...
export function sglib_intkv_node_delete(list, elem) {
    {
        let _p_ = null;
        for (_p_ = list; ((_p_.value != (null) && _p_.value != (elem)) ? 1 : 0); _p_ = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "intkv_node", "next_in_chain", 8))((_p_.value))) {
        }
...
export function sglib_intkv_node_delete_if_member(list, elem, member) {
    {
        let _p_ = null;
        for (_p_ = list; ((_p_.value != (null) && (i32(((_p_.value)).k - (__struct_ptr_at(((elem)), 0)).k)) != 0) ? 1 : 0); _p_ = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "intkv_node", "next_in_chain", 8))((_p_.value))) {
        }
        member.value = _p_.value;
        if (_p_.value != (null)) {
            _p_.value = (_p_.value).next_in_chain;
...
export function sglib_intkv_node_sort(list) {
    {
        let _r_ = null;
        let _a_ = null;
        let _b_ = null;
        ...
        for (_n_ = 1; _contFlag_; _n_ = i32(_n_ + _n_)) {
            ...
            while (_todo_ != (null)) {
                ...
                while (_restail_.value != (null)) {
                    _restail_ = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "intkv_node", "next_in_chain", 8))((_restail_.value));
                }
                _contFlag_ = 1;
            }
        }
...
export function sglib_intkv_node_len(list) {
    let res = 0;
    {
        let _ce_ = null;
        ((_ce_));
...
export function sglib_intkv_node_reverse(list) {
    {
        let _list_ = null;
        let _tmp_ = null;
        let _res_ = null;
...
export function sglib_intkv_node_it_init_on_equal(it, list, subcomparator, equalto) {
    (__struct_ptr_at(it, 0)).subcomparator = subcomparator;
    (__struct_ptr_at(it, 0)).equalto = equalto;
    (__struct_ptr_at(it, 0)).nextelem = list;
    return (sglib_intkv_node_it_next(it));
}
export function sglib_intkv_node_it_init(it, list) {
    return (sglib_intkv_node_it_init_on_equal(it, list, null, null));
}
export function sglib_intkv_node_it_current(it) {
    return ((__struct_ptr_at(it, 0)).currentelem);
}
export function sglib_intkv_node_it_next(it) {
    let ce = null;
    let eq = null;
    let scp = null;
    ce = (__struct_ptr_at(it, 0)).nextelem;
...
sglib_hashed_intkv_node_iterator.__fieldTypes = ["int32", "int64", "int32", "int32", "int64"];
sglib_hashed_intkv_node_iterator.__fieldNames = ["containerIt", "table", "currentIndex", "subcomparator", "equalto"];
sglib_hashed_intkv_node_iterator.__fieldOffsets = [0, 8, 16, 24, 32];
export function sglib_hashed_intkv_node_init(table) {
    let i = 0;
    for (i = ((0) >>> 0); ((i) >>> 0) < (((7)) >>> 0); (() => { const _t = i; i = u32(i + 1); return _t; })()) {
        cptr_write_ptr(table, ((i) >>> 0), null);
    }
}
export function sglib_hashed_intkv_node_add(table, elem) {
    let i = 0;
    i = u32((((Math.trunc(+(intkv_node_hash(elem)))) >>> 0)) % (((7)) >>> 0));
    sglib_intkv_node_add({ get value() { return cptr_read_ptr((table), ((i) >>> 0)); }, set value(__v) { cptr_write_ptr((table), ((i) >>> 0), __v); } }, elem);
}
export function sglib_hashed_intkv_node_add_if_not_member(table, elem, member) {
    let i = 0;
    i = u32((((Math.trunc(+(intkv_node_hash(elem)))) >>> 0)) % (((7)) >>> 0));
    return (sglib_intkv_node_add_if_not_member({ get value() { return cptr_read_ptr((table), ((i) >>> 0)); }, set value(__v) { cptr_write_ptr((table), ((i) >>> 0), __v); } }, elem, member));
}
export function sglib_hashed_intkv_node_delete(table, elem) {
    let i = 0;
    i = u32((((Math.trunc(+(intkv_node_hash(elem)))) >>> 0)) % (((7)) >>> 0));
    sglib_intkv_node_delete({ get value() { return cptr_read_ptr((table), ((i) >>> 0)); }, set value(__v) { cptr_write_ptr((table), ((i) >>> 0), __v); } }, elem);
}
export function sglib_hashed_intkv_node_delete_if_member(table, elem, memb) {
    let i = 0;
    i = u32((((Math.trunc(+(intkv_node_hash(elem)))) >>> 0)) % (((7)) >>> 0));
    return (sglib_intkv_node_delete_if_member({ get value() { return cptr_read_ptr((table), ((i) >>> 0)); }, set value(__v) { cptr_write_ptr((table), ((i) >>> 0), __v); } }, elem, memb));
}
export function sglib_hashed_intkv_node_is_member(table, elem) {
    let i = 0;
    i = u32((((Math.trunc(+(intkv_node_hash(elem)))) >>> 0)) % (((7)) >>> 0));
    return (sglib_intkv_node_is_member(cptr_read_ptr((table), ((i) >>> 0)), elem));
}
export function sglib_hashed_intkv_node_find_member(table, elem) {
    let i = 0;
    i = u32((((Math.trunc(+(intkv_node_hash(elem)))) >>> 0)) % (((7)) >>> 0));
    return (sglib_intkv_node_find_member(cptr_read_ptr((table), ((i) >>> 0)), elem));
}
export function sglib_hashed_intkv_node_it_init_on_equal(it, table, subcomparator, equalto) {
    let e = null;
...
export function sglib_hashed_intkv_node_it_init(it, table) {
    return (sglib_hashed_intkv_node_it_init_on_equal(it, table, null, null));
}
export function sglib_hashed_intkv_node_it_current(it) {
...
export function sglib_hashed_intkv_node_it_next(it) {
    let e = null;
...
sglib_int_node_sl_iterator.__fieldTypes = ["int64", "int64", "int32", "int64"];
sglib_int_node_sl_iterator.__fieldNames = ["currentelem", "nextelem", "subcomparator", "equalto"];
sglib_int_node_sl_iterator.__fieldOffsets = [0, 8, 16, 24];
export function sglib_int_node_sl_is_member(list, elem) {
    let result = 0;
    {
        let _p_ = null;
        for (_p_ = (list); ((_p_ != (null) && (i32((__struct_ptr_at((_p_), 0)).v - (__struct_ptr_at(((elem)), 0)).v)) < 0) ? 1 : 0); _p_ = (__struct_ptr_at(_p_, 0)).next) {
...
export function sglib_int_node_sl_find_member(list, elem) {
    let result = null;
    {
        let _p_ = null;
        let _cmpres_ = 1;
...
export function sglib_int_node_sl_add_if_not_member(list, elem, member) {
    {
        let _e_ = null;
        let _cmp_res_ = 0;
        {
            {
                (_cmp_res_) = -1;
                for ((_e_) = list; ((_e_.value != (null) && ((_cmp_res_) = (i32(((_e_.value)).v - (__struct_ptr_at(((elem)), 0)).v))) < 0) ? 1 : 0); (_e_) = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "int_node_sl", "next", 8))((_e_.value))) {
                }
            }
        }
        if (_cmp_res_ != 0) {
...
export function sglib_int_node_sl_add(list, elem) {
    {
        let _e_ = null;
        let _cmpres_ = 0;
        {
            {
                (_cmpres_) = -1;
                for ((_e_) = list; ((_e_.value != (null) && ((_cmpres_) = (i32(((_e_.value)).v - (__struct_ptr_at(((elem)), 0)).v))) < 0) ? 1 : 0); (_e_) = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "int_node_sl", "next", 8))((_e_.value))) {
                }
            }
        }
        (__struct_ptr_at((elem), 0)).next = _e_.value;
...
export function sglib_int_node_sl_delete(list, elem) {
    {
        {
            {
                let _p_ = null;
                for (_p_ = list; ((_p_.value != (null) && _p_.value != (elem)) ? 1 : 0); _p_ = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "int_node_sl", "next", 8))((_p_.value))) {
                }
...
export function sglib_int_node_sl_delete_if_member(list, elem, member) {
    {
        let _e_ = null;
        let _cmp_res_ = 0;
        {
...
export function sglib_int_node_sl_len(list) {
    let res = 0;
    {
        {
            {
...
export function sglib_int_node_sl_sort(list) {
    {
        let _r_ = null;
        let _a_ = null;
        let _b_ = null;
        ...
        for (_n_ = 1; _contFlag_; _n_ = i32(_n_ + _n_)) {
            ...
            while (_todo_ != (null)) {
                ...
                while (_restail_.value != (null)) {
                    _restail_ = ((__cur) => __field_ref_scalar(() => __struct_ptr_at(__cur, 0), "int_node_sl", "next", 8))((_restail_.value));
                }
                _contFlag_ = 1;
            }
        }
...
export function sglib_int_node_sl_it_init_on_equal(it, list, subcomparator, equalto) {
    (__struct_ptr_at(it, 0)).subcomparator = subcomparator;
    (__struct_ptr_at(it, 0)).equalto = equalto;
    (__struct_ptr_at(it, 0)).nextelem = list;
    return (sglib_int_node_sl_it_next(it));
}
export function sglib_int_node_sl_it_init(it, list) {
    return (sglib_int_node_sl_it_init_on_equal(it, list, null, null));
}
export function sglib_int_node_sl_it_current(it) {
    return ((__struct_ptr_at(it, 0)).currentelem);
}
export function sglib_int_node_sl_it_next(it) {
    let ce = null;
    let eq = null;
    let scp = null;
    let c = 0;
...
export function sglib_full_smoke() {
...
export function sglib_full_probe_int_list_len() {
    let a = Object.assign(new int_node(), { v: 1, next: null, prev: null, left: null, right: null, color: 0 });
    let b = Object.assign(new int_node(), { v: 2, next: null, prev: null, left: null, right: null, color: 0 });
    let h = null; /* &ref */
...
export function sglib_full_probe_int_list_reverse_first() {
    let a = Object.assign(new int_node(), { v: 1, next: null, prev: null, left: null, right: null, color: 0 });
    let b = Object.assign(new int_node(), { v: 2, next: null, prev: null, left: null, right: null, color: 0 });
    let c = Object.assign(new int_node(), { v: 3, next: null, prev: null, left: null, right: null, color: 0 });
    let h = null; /* &ref */
...
export function sglib_full_probe_dl_first_last() {
...
export function sglib_full_probe_str_list_len() {
    let s1 = Object.assign(new str_node_list(), { key: "alpha", next: null });
    let s2 = Object.assign(new str_node_list(), { key: "beta", next: null });
...
export function sglib_full_probe_dl_len() {
...
export function sglib_full_probe_rbtree_find() {
    let r5 = Object.assign(new int_node_rb(), { v: 5, left: null, right: null, color: 0 });
    let r3 = Object.assign(new int_node_rb(), { v: 3, left: null, right: null, color: 0 });
    let r8 = Object.assign(new int_node_rb(), { v: 8, left: null, right: null, color: 0 });
...
export function sglib_full_probe_hash_find() {
    let h1 = Object.assign(new intkv_node(), { k: 1, v: 100, next_in_chain: null });
    let h2 = Object.assign(new intkv_node(), { k: 2, v: 200, next_in_chain: null });
    let h3 = Object.assign(new intkv_node(), { k: 3, v: 300, next_in_chain: null });
    let htab = new Array(7).fill(null);
...
export function sglib_full_probe_sorted_head() {

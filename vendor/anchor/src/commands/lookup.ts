...
export async function handleLookup(argv: string[]): Promise<CliResult> {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg.startsWith("-")) {
      return { exitCode: 1, output: `anchor lookup: unknown flag '${arg}'` };
    }
    if (inputPath === undefined) { inputPath = arg; continue; }
    return { exitCode: 1, output: `anchor lookup: unexpected argument '${arg}'` };
  }

  if (inputPath === undefined) {
    return { exitCode: 1, output: "anchor lookup: missing required <map> argument" };
  }

  const modes = [targetSpec, sourceSpec, regionId].filter(Boolean).length;
  if (modes === 0) {
    return { exitCode: 1, output: "anchor lookup: specify --target, --source, or --region" };
  }
  if (modes > 1) {
    return { exitCode: 1, output: "anchor lookup: specify only one of --target, --source, or --region" };
  }
  ...
  try {
    ...
    if (regionId !== undefined) {
      ...
      if (entry === undefined) {
        return { exitCode: 1, output: `anchor lookup: region '${regionId}' not found` };
      }
      return {
        exitCode: 0,
        output: [
    ...
    if (targetSpec !== undefined) {
      ...
      if (parsed === null) {
        return { exitCode: 1, output: `anchor lookup: invalid --target format, expected file:line:col` };
      }
    ...
    if (sourceSpec !== undefined) {
      ...
      if (parsed === null) {
        return { exitCode: 1, output: `anchor lookup: invalid --source format, expected file:line:col` };
      }
      const pos = makePosition(parsed.line, parsed.column, 0);
    ...
    return { exitCode: 1, output: "anchor lookup: internal error" };
  } catch (error) {
    if (error instanceof AnchorError || error instanceof Error) {
      return { exitCode: 1, output: `anchor lookup: ${error.message}` };
    }
    throw error;
  }
}

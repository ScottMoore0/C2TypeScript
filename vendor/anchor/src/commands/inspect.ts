...
export async function handleInspect(argv: string[]): Promise<CliResult> {
  ...
  for (const arg of argv) {
    if (arg.startsWith("-")) {
      return { exitCode: 1, output: `anchor inspect: unknown flag '${arg}'` };
    }
    if (inputPath === undefined) { inputPath = arg; continue; }
    return { exitCode: 1, output: `anchor inspect: unexpected argument '${arg}'` };
  }
  if (inputPath === undefined) {
    return { exitCode: 1, output: "anchor inspect: missing required <map> argument" };
  }
  try {
    ...
    if (error instanceof AnchorError || error instanceof Error) {
      return { exitCode: 1, output: `anchor inspect: ${error.message}` };
    }
    throw error;
  }
}

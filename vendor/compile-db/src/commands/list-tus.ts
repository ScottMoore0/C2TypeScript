...
// Handler for `compile-db list-tus <path> [--format text|json]`.
...
export async function handleListTus(argv: string[]): Promise<CliResult> {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--format") {
      ...
      if (next !== "text" && next !== "json") {
        return {
          exitCode: 1,
          output: "compile-db list-tus: --format must be 'text' or 'json'"
        };
      }
      format = next;
      continue;
    ...
    if (arg.startsWith("-")) {
      return { exitCode: 1, output: `compile-db list-tus: unknown flag '${arg}'` };
    }
    ...
    return {
      exitCode: 1,
      output: `compile-db list-tus: unexpected argument '${arg}'`
    };
  }
  ...
  if (inputPath === undefined) {
    return {
      exitCode: 1,
      output: "compile-db list-tus: missing required <path> argument"
    };
  }
  ...
  try {
    ...
    if (error instanceof DossierError) {
      return { exitCode: 1, output: `compile-db list-tus: ${error.message}` };
    }
    throw error;
  }
}

...
// Handler for `dossier ingest <path> [--output <path>] [--variant-label <label>] [--project-root <path>]`.
//
...
export async function handleIngest(argv: string[]): Promise<CliResult> {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--output" || arg === "-o") {
      const next = argv[++i];
      if (next === undefined) {
        return { exitCode: 1, output: "dossier ingest: --output requires a path" };
      }
      outputPath = next;
      continue;
    }
    if (arg === "--variant-label") {
      const next = argv[++i];
      if (next === undefined) {
        return { exitCode: 1, output: "dossier ingest: --variant-label requires a value" };
      }
      variantLabel = next;
      continue;
    }
    if (arg === "--project-root") {
      const next = argv[++i];
      if (next === undefined) {
        return { exitCode: 1, output: "dossier ingest: --project-root requires a path" };
      }
      projectRootOverride = next;
      continue;
    }
    if (arg.startsWith("-")) {
      return { exitCode: 1, output: `dossier ingest: unknown flag '${arg}'` };
    }
    ...
    return {
      exitCode: 1,
      output: `dossier ingest: unexpected positional argument '${arg}'`
    };
  }
  ...
  if (inputPath === undefined) {
    return {
      exitCode: 1,
      output: "dossier ingest: missing required <path> argument"
    };
  }
  ...
  try {
    ...
    if (!existsSync(resolvedInput)) {
      return {
        exitCode: 1,
        output: `dossier ingest: path does not exist: ${inputPath}`
      };
    }
    ...
    if (statSync(resolvedInput).isFile()) {
      ...
      if (!found) {
        return {
          exitCode: 1,
          output: `dossier ingest: no compile_commands.json found in ${inputPath} or its common build subdirectories`
        };
      }
      compileDbPath = found;
      resolvedProjectRoot = projectRootOverride
    ...
    if (
      ...
      return { exitCode: 1, output: `dossier ingest: ${error.message}` };
    }
    throw error;
  }
}

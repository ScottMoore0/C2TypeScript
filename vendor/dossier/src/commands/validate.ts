...
// Handler for `dossier validate <path>`.
//
...
export async function handleValidate(argv: string[]): Promise<CliResult> {
  ...
  for (const arg of argv) {
    if (arg.startsWith("-")) {
      return { exitCode: 1, output: `dossier validate: unknown flag '${arg}'` };
    }
    ...
    return {
      exitCode: 1,
      output: `dossier validate: unexpected argument '${arg}'`
    };
  }
  ...
  if (inputPath === undefined) {
    return {
      exitCode: 1,
      output: "dossier validate: missing required <path> argument"
    };
  }
  ...
  try {
    ...
    if (error instanceof DossierError) {
      return { exitCode: 1, output: `dossier validate: ${error.message}` };
    }
    if (error instanceof Error && "code" in error) {
      return {
        exitCode: 1,
        output: `dossier validate: ${(error as Error).message}`
      };
    }
    throw error;
  }

...
async function doFetch(url: string, init: any): Promise<{ status: number; body: Uint8Array; headers: Record<string, string> }> {
  ...
  if (!hasFetch()) {
    const G2: any = G;
    if (!G2.__prismSuppressFetchWarnings) {
      G2.__prismSuppressFetchWarnings = true;
      try { (G.console ?? console).warn("[prism] FetchNetworkAdapter: fetch() unavailable — returning 0/empty"); } catch { /* ignore */ }
    }
...
export function openWebSocket(url: string, protocols?: string | string[]): WSClient {
  ...
  if (typeof G.WebSocket !== "function") {
    if (!G.__prismSuppressWSWarnings) {
      G.__prismSuppressWSWarnings = true;
      try { (G.console ?? console).warn("[prism] openWebSocket: WebSocket unavailable — returning stub"); } catch { /* ignore */ }
    }

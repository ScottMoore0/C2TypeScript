...
export async function handleVerify(argv: string[]): Promise<CliResult> {
  ...
  for (const arg of argv) {
    if (arg.startsWith("-")) {
      return { exitCode: 1, output: `source-mapping verify: unknown flag '${arg}'` };
    }
    if (inputPath === undefined) { inputPath = arg; continue; }
    return { exitCode: 1, output: `source-mapping verify: unexpected argument '${arg}'` };
  }
  if (inputPath === undefined) {
    return { exitCode: 1, output: "source-mapping verify: missing required <map> argument" };
  }
  try {
    ...
    if (error instanceof AnchorError) {
      return { exitCode: 1, output: `source-mapping verify: ${error.message}` };
    }

...
export async function handleListRegions(argv: string[]): Promise<CliResult> {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--format") {
      ...
      if (next !== "text" && next !== "json") {
        return { exitCode: 1, output: "source-mapping list-regions: --format must be 'text' or 'json'" };
      }
      format = next; continue;
    }
    if (arg.startsWith("-")) {
      return { exitCode: 1, output: `source-mapping list-regions: unknown flag '${arg}'` };
    }
    if (inputPath === undefined) { inputPath = arg; continue; }
    return { exitCode: 1, output: `source-mapping list-regions: unexpected argument '${arg}'` };
  }

  if (inputPath === undefined) {
    return { exitCode: 1, output: "source-mapping list-regions: missing required <map> argument" };
  }
  ...
  try {
    ...
    if (error instanceof AnchorError || error instanceof Error) {
      return { exitCode: 1, output: `source-mapping list-regions: ${error.message}` };
    }
    throw error;
  }
}

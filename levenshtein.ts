...
export function levenshtein_n(a: any, length: number, b: any, bLength: number): number {
  if (typeof a === 'string') a = cptr_from_string(a);
  if (typeof b === 'string') b = cptr_from_string(b);
  ...
  if ((((bLength) >>> 0) == ((0) >>> 0) ? 1 : 0)) {
    return ((length) >>> 0);
  }
  let cache = calloc(((length) >>> 0), 8); /* &ref */
  let index = ((0) >>> 0);
  let bIndex = ((0) >>> 0);
  let distance = 0;
  let bDistance = 0;
  let result = 0;
  let code = 0;
  while ((((index) >>> 0) < ((length) >>> 0) ? 1 : 0)) {
    cptr_write_uint32(cache, ((index) >>> 0), ((index) >>> 0) + ((1) >>> 0));
    (() => { const _t = index; index = u32(index + 1); return _t; })();
  }
  while ((((bIndex) >>> 0) < ((bLength) >>> 0) ? 1 : 0)) {
    code = (((b.buf[(b.off ?? 0) + ((bIndex) >>> 0)]) << 24 >> 24)) << 24 >> 24;
    result = distance = (() => { const _t = bIndex; bIndex = u32(bIndex + 1); return _t; })();
    index = Number(18446744073709551615n);
    while (((index = u32(index + 1)) < ((length) >>> 0) ? 1 : 0)) {
      bDistance = ((((code) << 24 >> 24) == ((a.buf[(a.off ?? 0) + ((index) >>> 0)]) << 24 >> 24) ? 1 : 0) ? ((distance) >>> 0) : ((distance) >>> 0) + ((1) >>> 0));
      distance = ((cptr_read_uint32(cache, ((index) >>> 0))) >>> 0);
      cptr_write_uint32(cache, ((index) >>> 0), result = ((((distance) >>> 0) > ((result) >>> 0) ? 1 : 0) ? ((((bDistance) >>> 0) > ((result) >>> 0) ? 1 : 0) ? ((result) >>> 0) + ((1) >>> 0) : ((bDistance) >>> 0)) : ((((bDistance) >>> 0) > ((distance) >>> 0) ? 1 : 0) ? ((distance) >>> 0) + ((1) >>> 0) : ((bDistance) >>> 0))));
    }
  }
  free(cache);
  return ((result) >>> 0);
...
export function levenshtein(a: any, b: any): number {
  let length = strlen(cptr_clone(a));
  let bLength = strlen(cptr_clone(b));
  return levenshtein_n(cptr_clone(a), ((length) >>> 0), cptr_clone(b), ((bLength) >>> 0));
}
